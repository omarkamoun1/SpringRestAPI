...
class DefaultRequestHandlerCombiner implements RequestHandlerCombiner {
	...
	private Collection<? extends RequestHandler> combined(Collection<RequestHandler> requestHandlers) {
		...
		return byPatternsCondition().compound(byOperationName()).sortedCopy(combined);
	}
	...
	private ImmutableListMultimap<Equivalence.Wrapper<RequestHandler>, RequestHandler> safeGroupBy(List<RequestHandler> source) {
		try {
			...
			LOGGER.error("Unable to index request handlers {}. Request handlers with issues{}", e.getMessage(), keys(source));

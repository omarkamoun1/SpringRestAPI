...
public class DocumentationPluginsBootstrapper implements SmartLifecycle {
  ...
  private DocumentationContextBuilder defaultContextBuilder(DocumentationPlugin plugin) {
    ...
    return documentationPluginsManager
        ...
        .rules(rules)
        .requestHandlers(combiner(plugin).combine(requestHandlers));
  }
  ...
  private RequestHandlerCombiner combiner( DocumentationPlugin plugin) {
    return Optional.fromNullable(combiner).or(new DefaultRequestHandlerCombiner(  plugin));
  }

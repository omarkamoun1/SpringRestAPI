...
@JsonPropertyOrder({ "name", "in", "description", "required", "type", "items", "collectionFormat", "default", "maximum", "exclusiveMaximum", "minimum", "exclusiveMinimum", "maxLength", "minLength", "pattern", "maxItems", "minItems", "uniqueItems",
		"multipleOf" })
public abstract class AbstractSerializableParameter<T extends AbstractSerializableParameter<T>> extends AbstractParameter implements SerializableParameter {
	...
	// LoggerFactory.getLogger(AbstractSerializableParameter.class);
		...
		if (BaseIntegerProperty.TYPE.equals(type) || DecimalProperty.TYPE.equals(type)) {
			for (String s : _enum) {
				try {
					...
					// LOGGER.warn(String.format("Illegal enum value %s for
					// parameter type %s", s, type), e);
					oList.add(s);
				}
			}
	...
	public Object getDefault() {
		...
		try {
			...
			// LOGGER.warn(String.format("Illegal DefaultValue %s for parameter
			// type %s", defaultValue, type), e);
		}
		return defaultValue;
	}

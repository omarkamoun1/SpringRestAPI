		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.companyid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"+params[i]+"') \"" + 
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.customerid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"+params[i]+"') \"" + 
						(colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + 
						"\"");
			}
		}
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.candidateid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"+params[i]+"') \"" + 
						(colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + 
						"\"");
			}
		}
...
//		System.out.println(sql);
	...
	public static String CandidateResumeSourceDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			...
			" from tcandidatedocument_header h , tcandidate c, (select text from (select text from gdpr_inaccessibility_text where teamid = ? or teamid = 0 order by teamid desc) where rownum = 1) d " +
	...
	public static String CandidatesResumesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			...
			"    and a.teamid= ? and a.candidateid = b.id and bitand(nvl(b.privacyapplied,0), 32) = 0 " + 
			"    union all " + 
			"    select a.candidateid, null, null, null, null, null, null, null " + 
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.startid=a.id and t.teamid=a.recruiter_teamid and n.teamid=a.recruiter_teamid and n.id=t.userfield_id and n.fieldname='"+params[i]+"') \"" + 
						(colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + 
						"\"");
			}
		}
...
//		StringBuffer hotlistsToShow = new StringBuffer();
		String sql = "select h.hotlist_id, h.contactid, c.firstname, c.lastname, c.title, c.companyname, c.workphone, c.email " +
				"from tcontact_hotlist_contacts h, tcustomer c " +
...
//		StringBuffer hotlistsToShow = new StringBuffer();
	...
	public static String adpProfiles(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql = 
				...
				"   decode(LOWER(a.federalmarritalstatus),'m','M - Married','s','S - Single','') federal_marital_status, " + 

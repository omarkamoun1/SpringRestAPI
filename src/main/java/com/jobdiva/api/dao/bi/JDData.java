...
public class JDData {
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.companyid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"
						+ StringUtils.escapeSql(params[i]) + "' AND (ROWNUM = 1)  ) "
						+ "\"" + (colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + "\"");
			}
		}
		String sql =
			"select id, name companyname, dateentered, a.RECRUITERID USERID, editdate dateupdated, userfield_updatedate dateuserfieldupdated, parent_companyid parentcompanyid, parent_company_name parentcompanyname, " +
			...
			"  submittal_instr submittal_guidelines " +
			columnsToShow.toString() + " from tcustomercompany a, tcustomercompanyaddresses b where a.id=? and a.teamid=? " +
			" and b.companyid=a.id and b.default_address=1 ";
	...
	public static String CompaniesDetail(Long clientID, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				try { Long.parseLong(params[i]); noRec++; } catch(Exception e) { noRec = i; break; }
			}
		...
		String sql =
			"select id, name companyname, dateentered, editdate dateupdated, userfield_updatedate dateuserfieldupdated, parent_companyid parentcompanyid, parent_company_name parentcompanyname, " +
			"  b.address1, b.address2, b.city, b.state, b.zipcode, b.phone, b.fax, b.email, b.url, " +
			"  discount, decode(discount_type,0,'$/Hour',1,'$/Day',2,'$/Month',3,'$/Year','') discounttype, discountpct, " +
			"  decode(invoice_contact_preference,1,'Email',2,'Do Not Send','Address') show_on_invoice, " +
			"  decode(entryformat,1,'Minute',2,'Quarter',3,'Time In/Time Out',4,'Default - When Billing Unit is Daily','Default') timesheet_entryformat, " +
			...
			"  decode(financial_expensecategorytotal,1,'Display Only Category Totals','Display Expense Details') expensecategorytotal, " +
			"  submittal_instr submittal_guidelines " +
			columnsToShow.toString() + " from tcustomercompany a, tcustomercompanyaddresses b " +
	...
	public static String CompanyOwnersDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select a.recruiterid userid, b.firstname, b.lastname, isprimaryowner " +
			" from tcustomer_company_owners a, trecruiter b " +
			" where a.companyid=? and a.teamid=? and b.id=a.recruiterid ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
	}
	...
	public static String CompaniesOwnersDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select a.companyid, a.recruiterid userid, b.firstname, b.lastname, isprimaryowner " +
			" from tcustomer_company_owners a, trecruiter b " +
	...
	public static String CompaniesTypesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select a.companyid, b.typename " +
			" from tcustomer_company_type a, tcustomer_company_types b " +
			" where a.companyid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) and a.teamid=? " +
	...
	public static String CompanyAddressesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select companyid, addressid, default_address, address1, address2, city, state, zipcode, countryid, phone, fax, url, email " +
			" from tcustomercompanyaddresses a " +
			" where a.companyid=? and a.teamid=? " +
			" order by addressid ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.customerid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"
						+ StringUtils.escapeSql(params[i]) + "' AND (ROWNUM = 1) ) "
						+ "\"" + (colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + "\"");
			}
		}
		String sql =
			"select a.id, firstname, lastname, companyid, companyname, departmentname, title, active, timeentered datecreated, a.RECRUITERID USERID, editdate dateupdated, userfield_updatedate dateuserfieldupdated," +
	...
	public static String V2ContactDetail(Long clientID, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			"select a.id, firstname, lastname, companyid, companyname, a.RECRUITERID USERID, departmentname, title, active, " +
	...
	public static String CandidateDetail(Long clientID, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
			...
			for(int i = 1; i < params.length; i++) {
				if(!list.contains(params [i]) )  {
					...
					columnsToShow.append(", (select " +
						...
						" where t.candidateid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"
						+ StringUtils.escapeSql(params[i]) + "' AND (ROWNUM = 1) ) "
						+ "\"" + (colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + "\""  ) ; //
		...
		String sql =
			"select id, decode(bitand(nvl(privacyapplied,0), 1), 0, firstname, b.text) firstname, decode(bitand(nvl(privacyapplied,0), 1), 0, lastname, b.text) lastname," +
	...
	public static String CandidateQualDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select a.candidateid, b.catname qualname, c.dcatname qualvalue, a.datecreated dateentered " +
...
//		System.out.println(sql);
	...
	public static String CandidateHRDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql = "select a.candidateid, a.legal_firstname, a.legal_middlename, a.legal_lastname, a.generationsuffix, a.ssn, to_char(a.dateofbirth, 'mm/dd/yyyy') as dateofbirth,"
				...
				+ " a.visastatus, a.userfield_updatedate dateuserfieldupdated "
				+ " from tcandidate_hr a, tcandidate b"
	...
	public static String CandidatesQualDetail(Long clientID, String[] params, Vector<Object> param) {
		...
		String sql =
			"select a.candidateid, b.catname qualname, c.dcatname qualvalue, a.datecreated dateentered " +
			" from tcandidate_category a, tcategories b, tcategories_detail c " +
			" where a.candidateid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) and a.teamid=? and a.dirty<>2 " +
			"   and b.teamid=a.teamid and b.catid=a.catid " +
			"   and c.teamid=a.teamid and c.catid=a.catid and c.dcatid=a.dcatid " +
			"   and (";
		...
		if(params.length == noRec) sql += "1=1";
		sql += ") order by qualname";
...
//		System.out.println(sql);
	...
	public static String newUpdatedCandidateQuals(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			"select candidateid, b.catname qualname, c.dcatname qualvalue, a.datecreated dateentered, 0 deleteflag " +
			"from tcandidate_category a, tcategories b, tcategories_detail c " +
			" where a.teamid=? and datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') and a.dirty<>2 " +
			"   and b.teamid=a.teamid and b.catid=a.catid " +
			"   and c.teamid=a.teamid and c.catid=a.catid and c.dcatid=a.dcatid " +
			"  union all " +
			"select candidateid, b.catname qualname, c.dcatname qualvalue, a.datedeleted dateentered, 1 deleteflag " +
			"from tcandidatequal_deleted a, tcategories b, tcategories_detail c " +
			" where a.teamid=? and datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"   and b.teamid=a.teamid and b.catid=a.catid " +
			"   and c.teamid=a.teamid and c.catid=a.catid and c.dcatid=a.dcatid ";;
		param.add(clientID);
		param.add(fromDate);
	...
	public static String CandidateResumeSourceDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select h.candidateid, h.docid, h.datecreated dateuploaded, h.datereceived datedownloaded, " + 
			...
			"(select s.name from tresumesourcename s where s.teamid=h.teamid and s.id=h.resumesource) sourcename, " + 
			"(select r.firstname||' '||r.lastname from trecruiter r where r.id=h.recruiterid and r.groupid=h.teamid) recruitername " + 
			"from tcandidatedocument_header h , tcandidate c where h.candidateid=? and h.teamid=? and h.candidateid = c.id and bitand(nvl(c.privacyapplied,0), 32) = 0 " + 
			"and h.teamid = c.teamid " + 
			"union all " + 
			...
			"where h.candidateid= ? and h.teamid= ? and h.candidateid = c.id and bitand(nvl(c.privacyapplied,0), 32) > 0 " + 
			"and h.teamid = c.teamid and h.teamid = d.teamid";
		param.add(new Long(params[0]));
		param.add(clientID);
		param.add(clientID);
		param.add(new Long(params[0]));
	...
	public static String CandidateResumesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select a.candidateid, a.docid, a.dbid, a.global_id resumeid, a.datecreated_original datecreated, a.datecreated dateupdated, a.datereceived_original datefirstdownloaded,  a.datereceived datelastdownloaded " +
			" from tcandidatedocument_header a , tcandidate b" +
			" where a.candidateid= ? and a.teamid= ? and a.teamid = b.teamid and a.candidateid = b.id and bitand(nvl(b.privacyapplied,0), 32) = 0" +
			" union all" +
			" select a.candidateid, null, null, null, null, null, null, null" +
			" from tcandidatedocument_header a , tcandidate b " +
			" where a.candidateid=? and a.teamid=? and a.teamid = b.teamid and a.candidateid = b.id and bitand(nvl(b.privacyapplied,0), 32) > 0";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String CandidatesResumesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"SELECT * FROM ( " + 
			"    select a.candidateid, a.docid, a.dbid, a.global_id resumeid, a.datecreated_original datecreated, a.datecreated dateupdated, " + 
			"    a.datereceived_original datefirstdownloaded, a.datereceived datelastdownloaded " + 
			"    from tcandidatedocument_header a , tcandidate b " + 
			"    where a.candidateid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) " + 
			"    and a.teamid= ? and a.teamid = b.teamid and a.candidateid = b.id and bitand(nvl(b.privacyapplied,0), 32) = 0 " + 
			"    union all " + 
			"    select a.candidateid, null, null, null, null, null, null, null " + 
			"    from tcandidatedocument_header a , tcandidate b, (select text from (select text from gdpr_inaccessibility_text " + 
			"    where teamid = ? or teamid = 0 order by teamid desc) where rownum = 1) c " + 
			"    where a.candidateid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) " + 
			"    and a.teamid=? and a.teamid = b.teamid and a.candidateid = b.id and bitand(nvl(b.privacyapplied,0), 32) > 0) " + 
	...
	public static String CandidateResumeDetail2(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select candidateid, docid, datecreated, datecreated_original, datereceived, datereceived_original " +
			"from tcandidatedocument_header " +
			"where candiateid=? and teamid=?";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String CandidateOnboardingDocumentDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select b.id, b.filename, ' ' filecontent, a.datecreated assignedon, uploadedon from tcandidate_onboarding a, tcandidate_onboarding_docs b " +
			"where a.candidateid=? and a.interviewid=? and a.teamid=? and lower(a.filename) = ? " +
	...
	public static String CandidateAttachmentDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select id, filename, ' ' filecontent, datecreated uploadedon from tcandidate_attachments " +
			"where candidateid=? and teamid=? and lower(filename) = ? ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String CompanyNoteDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select companyid, noteid, case when note is null then to_char(note_clob) else note end note " +
			" from tcustomercompanynotes where noteid=? and recruiter_teamid=? ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String CompanyNotesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select companyid, noteid, case when note is null then to_char(note_clob) else note end note " +
			" from tcustomercompanynotes " +
			"where noteid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) " +
			"  and recruiter_teamid=? ";
		StringBuilder noteids = new StringBuilder();
		for(int i = 0; i < params.length; i++) {
			if(i > 0) noteids.append(",");
	...
	public static String ContactNoteDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select note " +
			" from tcustomernotes where noteid=? and recruiter_teamid=? ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String ContactNotesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select noteid, customerid contactid, note " +
			" from tcustomernotes " +
			"where noteid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) " +
			"  and recruiter_teamid=? ";
		StringBuilder noteids = new StringBuilder();
	...
	public static String CandidateNoteDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select candidateid, noteid, case when note_clob is null or length(note_clob) = 0 then note else sf_htmlclob2text(note_clob) end note " +
			" from tcandidatenotes where noteid=? and recruiter_teamid=? ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String CandidateAvailabilityDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select candidateid, case when flag=2 then 1 else 0 end unavailableindef, dateavailable, " +
			" DATE_CREATED datecreated " +
			" from tcandidate_unreachable where candidateid=? and teamid=?" +
			" union " +
			"select h.candidateid, 0 unavailableindef, max(h.datecreated) dateavailable, " +
			" max(h.DATECREATED) datecreated " +
			" from tcandidatedocument_header h where h.candidateid=? and h.teamid=? " +
	...
	public static String CandidatesAvailabilityDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select candidateid, case when flag=2 then 1 else 0 end unavailableindef, dateavailable, " +
			" DATE_CREATED datecreated " + 
			" from tcandidate_unreachable where candidateid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type) from dual )) and teamid=?" +
			"  union " +
			"select h.candidateid, 0 unavailableindef, max(h.datecreated) dateavailable, " +
			" max(h.DATECREATED) datecreated " + 
	...
	public static String CandidateEEODetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			"select te.candidateid, e.name eeocategory, nvl(ed.name,'Unspecified') selection " +
			"from tcandidate_eeo te, teeo e , teeo_detail ed " +
			"where te.candidateid=? and te.teamid=? and te.eeoid <> 3 " +
			"and e.id=te.eeoid and ed.eeoid(+)=te.eeoid and ed.id(+)=te.deeoid " +
			"union all " +
			"select te.candidateid, e.name eeocategory, ed.name selection " +
			"from tcandidate_eeo te, teeo e , teeo_detail ed " +
			"where te.candidateid=? and te.teamid=? and te.eeoid = 3 " +
			"and e.id=te.eeoid and ed.eeoid=te.eeoid and bitand(te.deeoid,ed.id)=ed.id ";
		param.add(new Long(params[0]));
		param.add(clientID);
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String ContactOwnersDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select a.customerid contactid, a.recruiterid userid, b.firstname, b.lastname, isprimaryowner " +
			" from tcustomer_owners a, trecruiter b " +
			" where a.customerid=? and a.teamid=? and b.id=a.recruiterid ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
	}
	...
	public static String ContactsTypesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select a.id contactid, b.typename " +
			" from tcustomer a, tcustomertype b " +
			" where a.id in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) and a.teamid=? " +
	...
	public static String ContactAddressesDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select contactid, id addressid, default_address, address1, address2, city, state, zipcode, countryid, freetext label " +
			" from tcustomeraddress a " +
			" where a.contactid=? and a.teamid=? and (deleted is null or deleted = 0) " +
			" order by addressid ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
	...
	public static String JobDetail(Long clientID,  String[] params, Vector<Object> param, String[] restriction, Map<String, String> colNameToAliasMap) {
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.rfqid=a.id and t.teamid=a.teamid and n.teamid=a.teamid and n.id=t.userfield_id and n.fieldname='"
						+ StringUtils.escapeSql(params[i]) + "' AND (ROWNUM = 1) ) "
						+ "\"" + (colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + "\"");
			}
		}
		String sql =
			"select  a.id, a.dateissued, a.datelastupdated dateupdated, a.userfield_updatedate dateuserfieldupdated, a.datestatusupdated, " + 
			...
			"  a.customerid, a.companyid, a.address1, a.address2, a.city, a.state, a.zipcode, " +
		...
		param.add(new Long(params[0]));
		param.add(clientID);
		if(restriction!=null && restriction.length>0 && restriction[0]!=null) param.add(new Long(restriction[0]));
	...
	public static String JobContactsDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			" select a.rfqid jobid, a.customerid, nvl(b.name,'Unspecified') role " +
			" from trfq_customers a, tcontactroles b " +
	...
	public static String JobUsersDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			" select a.rfqid jobid, a.recruiterid userid, b.firstname, b.lastname, " +
			"   sales, lead_sales primarysales, decode(sales+lead_sales,0,1,recruiter) recruiter, lead_recruiter primaryrecruiter, datelastassigned " +
			" from trecruiterrfq a, trecruiter b " +
			" where a.rfqid=? and a.teamid=? and b.id=a.recruiterid " +
			" order by a.sales desc, a.lead_sales desc ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
	}
	...
	public static String JobsUsersDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			" select /*+ index(a, PK_TRECRUITERRFQ) */ a.rfqid jobid, a.recruiterid userid, b.firstname, b.lastname, " +
			"   sales, lead_sales primarysales, decode(sales+lead_sales,0,1,recruiter) recruiter, lead_recruiter primaryrecruiter, datelastassigned " +
			" from trecruiterrfq a, trecruiter b " +
			" where a.rfqid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) and a.teamid=? and b.id=a.recruiterid " +
			" order by a.sales desc, a.lead_sales desc ";
		StringBuilder jobs = new StringBuilder();
		for(int i = 0; i < params.length; i++) {
			if(i > 0) jobs.append(",");
...
//		System.out.println("BIData get to here");
		String sql =
			...
			" from trecruiterrfq rr, trfq a, tdivision b, tcustomercompany c, tcustomer d, trecruiterrfq pr, trecruiterrfq ps " +
			" where rr.teamid=? and rr.recruiterid=? and rr.jobstatus=? and a.id=rr.rfqid " +
	...
	public static String usersTasksDetail(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			" select id, recruiterid, (select contactid from tevent_contacts where eventid=id and rownum=1) customerid, candidateid, creatorid, " +
			"   eventdate, title, decode(optional,2,'No',4,'Yes','') completed, " +
			"	(select b.firstname||' '||b.lastname from trecruiter b where b.id=tevent.recruiterid) assignedto, " +
			"	(select b.firstname||' '||b.lastname from trecruiter b where b.id=tevent.creatorid) createdby, " +
			"	case when result is null or length(trim(result))=0 " +
			"	then (select typename from ttask_type t where t.teamid=tevent.teamid and t.id=tevent.task_type) " +
			"	else (select typename from ttask_type_candidate t where t.teamid=tevent.teamid and t.id = task_type) end tasktype, " +
			"	decode(private,1,'Yes','No') private, datecreated, lastmodified " +
			" from tevent " +
			" where recruiterid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) " +
			"   and teamid=? and optional in (2,4) and status_new = 'ACTIVE' " +
	...
	public static String EventsAttendeesDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			" select a.eventid, a.recruiterid userid, case when a.recruiterid > 0 then b.firstname||' '||b.lastname else a.sync_displayname end attendeename, " +
			"  case when a.recruiterid > 0 then b.email else a.sync_email end attendeeemail, isorganizer, responsestatus " +
			" from trecruiterevent a, trecruiter b " +
...
//			System.out.println("BIData Jobs Users Detail, teamid = " + clientID + ", list of job IDs = " + jobs.toString());
	...
	public static String jobUserUpdated(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select jobid, max(dateupdated) dateupdated " +
			" from tjob_userupdated " +
			" where teamid =?  and dateupdated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			" group by jobid";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String jobsPosted(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select * from " +
			"(select rfqid as JOBID, RFQREFNO, b.websitename as WEBSITENAME, DATEPOSTED, DATEADDED, username as ACCOUNTNAME, " +
			"   case when postingerror is null then 'Success' else postingerror end as STATUS " +
			" from trfqposted a, twebsites b " +
			" where a.teamid =? and a.dateposted between str_to_date(?,'%m/%d/%Y %H:%i:%s') and str_to_date(?,'%m/%d/%Y %H:%i:%s') " +
			"   and b.id = a.siteid) x ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String jobsCurrentlyPosted(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			...
			"   case when postingerror is null then 'Success' else postingerror end as STATUS " +
			" from trfqposted a, twebsites b " +
			" where a.teamid =? and b.id = a.siteid) x ";
		param.add(clientID);
		return sql;
	}
	...
	public static String portalJobsList(Long clientID, String[] params, Vector<Object> param) {
		...
		String sql =
				" select id jobid, nvl(posting_date,dateissued) dateissued, datelastupdated dateupdated, userfield_updatedate dateuserfieldupdated, rfqtitle jobtitle " +
				" from trfq " +
				" where teamid = ? and jobstatus = 0 " +
				...
				"   and (skills is null or upper(skills) not like 'SKILLS TO BE ASSIGN%') " +
				" order by nvl(posting_date,dateissued) desc ";
		param.add(clientID);
		return sql;
	}
	...
	public static String JobApplicantsDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			" select a.candidateid, decode(bitand(nvl(c.privacyapplied,0), 1), 0, c.firstname, g.text) firstname, decode(bitand(nvl(c.privacyapplied,0), 1), 0, c.lastname, g.text) lastname, " +
			" decode(bitand(nvl(c.privacyapplied,0), 16), 0, c.email, g.text) email, a.rfqid jobid, a.dateapplied, " +
			"   decode(a.resume_source_name,'JOBDIVA',(select company from tteam where id=a.teamid),a.resume_source_name) resumesource," +
			"   case when r.daterejected is not null then 'Rejected' " +
			"        when exists (select 1 from tinterviewschedule i where i.candidateid=a.candidateid and i.rfqid=a.rfqid) then 'Submitted'" +
			"   else null end action, " +
			"   case when r.daterejected is not null then r.recruiterid " +
			"   else (select i.recruiterid from tinterviewschedule i where i.candidateid=a.candidateid and i.rfqid=a.rfqid " +
			"      and i.id = (select min(j.id) from tinterviewschedule j where j.candidateid=a.candidateid and j.rfqid=a.rfqid)) " +
			"   end actionuserid, " +
			"   case when r.daterejected is not null then r.daterejected " +
			"   else (select min(datepresented) from tinterviewschedule i where i.candidateid=a.candidateid and i.rfqid=a.rfqid) " +
			"   end actiondate, " +
			"   (select nvl(f.displayname, h.resumesourcerefname) from tcandidatedocument_header h, tresumesourceref f " +
			"    where h.candidateid=a.candidateid and h.teamid=a.teamid and h.docid=" +
			...
			"     then a.global_id else null end, (select h.global_id from tcandidatedocument_header h " +
			"     where h.candidateid=a.candidateid and h.teamid=a.teamid and h.docid=" +
			"       (select min(h1.docid) from tcandidatedocument_header h1 where h1.candidateid=h.candidateid and h1.teamid=h.teamid and h1.resumesourceflag=1 " +
			"        and (h1.docid=a.docid or h1.datereceived > a.dateapplied)))) resumeid " + 
			...
			" where a.teamid = ? and a.rfqid=? and a.dirty = 0 " +
			"   and c.id=a.candidateid and c.teamid=a.teamid " +
	...
	public static String CandidateJobApplicationDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			" select a.candidateid, a.rfqid jobid, decode(bitand(nvl(c.privacyapplied,0), 1), 0, c.firstname, g.text) firstname, decode(bitand(nvl(c.privacyapplied,0), 1), 0, c.lastname, g.text) lastname," +
			...
			" from tcandidate_applyforjob a, tcandidate c, trejection r, (select text from (select text from gdpr_inaccessibility_text where teamid = ? or teamid = 0 order by teamid desc) where rownum = 1) g " +
			" where a.teamid = ? and a.candidateid = ? and a.rfqid = ? and a.dirty = 0 " +
			"   and c.id=a.candidateid and c.teamid=a.teamid " +
			"   and r.candidateid(+)=a.candidateid and r.recruiter_teamid(+)=a.teamid and r.rfqid(+)=a.rfqid ";
		param.add(clientID);
		param.add(clientID);
	...
	public static String JobSubmittalsDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			"select id, recruiterid userid, primarysalesid, candidateid, customerid contactid, roleid, datecreated, dateupdated, placementdate, interviewscheduledate, " +
			"  datepresented submittaldate, " +
			" case when roleid > 900 then daterejected else extdaterejected end rejectiondate, " +
			"  dateinterview, interview_timezoneid, datehired startdate, date_ended enddate, a.dateterminated terminationdate, " +
			"  managerfirstname, managerlastname, " +
			" case when dateterminated is not null and reasonterminated > 0 " +
			"      then (select description from treason_termination x where x.teamid=a.recruiter_teamid and x.id=reasonterminated) " +
			"      else null end terminationreason, " +
			" case when EXTREJECTREASONID is not null and EXTREJECTREASONID > 0 " +
			"      then (select reason from TREJECTION_REASONS rejreason where rejreason.teamid=a.recruiter_teamid and rejreason.id=a.EXTREJECTREASONID) " +
			"      else null end rejectreason, " +
			" (case when a.hourly > 0 then a.hourly_corporate when a.daily > 0 then a.daily_corporate else a.yearly_corporate end) corptocorp, " +
			" pay_hourly agreedbillrate, " +
			" case when finalbillrateunit is not null " +
			"      then decode(finalbillrateunit,'h','Hourly','d','Daily','y','Yearly', " +
			"           (select x.name from trateunits x where x.teamid=a.recruiter_teamid and x.ratetype=1 and x.unitid=finalbillrateunit)) " +
			"      else null end billfrequency, " +
			" case when hourly > 0 then hourly when daily > 0 then daily else yearly end agreedpayrate, " +
			...
			"      else (case when hourly > 0 then 'Hourly' when daily > 0 then 'Daily' when yearly > 0 then 'Yearly' else null end) end payfrequency, " +
			" nvl((select x.name from tcurrency x where x.id=a.hourly_currency), " +
			...
			" case when a.datehired is not null and a.contract=1 and fee_type=1 then fee when contract=1 and fee_type=0 then yearly*fee/100 else null end fee, " +
			" notes " +
	...
	public static String JobsSubmittalsDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			"select a.rfqid jobid, id, recruiterid userid, primarysalesid, candidateid, customerid contactid, roleid, datecreated, dateupdated, " +
			"  datepresented submittaldate, placementdate, interviewscheduledate, " +
			" case when roleid > 900 then daterejected else extdaterejected end rejectiondate, " +
			"  dateinterview, interview_timezoneid, datehired startdate, date_ended enddate, a.dateterminated terminationdate, " +
			"  managerfirstname, managerlastname, " +
			" case when dateterminated is not null and reasonterminated > 0 " +
	...
	public static String CandidateSubmittalsDetail(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
			"select id, recruiterid userid, primarysalesid, rfqid jobid, customerid contactid, roleid, datecreated, dateupdated, " +
			"  datepresented submittaldate, " +
			" case when roleid > 900 then daterejected else extdaterejected end rejectiondate, " +
			"  dateinterview, interview_timezoneid, datehired startdate, date_ended enddate, a.dateterminated terminationdate, " +
			"  managerfirstname, managerlastname, " +
		...
		if(params != null) {
			for(int i = 1; i < params.length; i++) {
				columnsToShow.append(", (select " +
						...
						" where t.startid=a.id and t.teamid=a.recruiter_teamid and n.teamid=a.recruiter_teamid and n.id=t.userfield_id and n.fieldname='"
						+ StringUtils.escapeSql(params[i]) + "' AND (ROWNUM = 1) ) "
						+ "\"" + (colNameToAliasMap.containsKey(params[i])? colNameToAliasMap.get(params[i]) : params[i]) + "\"");
			}
		}
		String sql =
			"select id, recruiterid userid, primarysalesid, candidateid, rfqid jobid, customerid contactid, roleid, datecreated, dateupdated, userfield_updatedate dateuserfieldupdated, " +
			"  datepresented submittaldate, placementdate, interviewscheduledate, " +
			" case when roleid > 900 then daterejected else extdaterejected end rejectiondate, " +
			"  dateinterview, interview_timezoneid, datehired startdate, date_ended enddate, a.dateterminated terminationdate, " +
			"  managerfirstname, managerlastname, " +
			...
			" case when a.datehired is not null and a.contract=1 and fee_type=1 then fee when a.datehired is not null and contract=1 and fee_type=0 then yearly*fee/100 else null end fee, " +
			" notes " +
			columnsToShow.toString() + " from tinterviewschedule a where a.id=? and a.recruiter_teamid=? ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String SubmittalsDetail(Long clientID,  String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			"select id, recruiterid userid, primarysalesid, candidateid, rfqid jobid, customerid contactid, roleid, datecreated, dateupdated, userfield_updatedate dateuserfieldupdated, " +
			" datepresented submittaldate, placementdate, interviewscheduledate, " +
			" case when roleid > 900 then daterejected else extdaterejected end rejectdate, " +
			" dateinterview, interview_timezoneid, datehired startdate, date_ended enddate, a.dateterminated terminationdate, " +
			"  managerfirstname, managerlastname, " +
			" case when dateterminated is not null and reasonterminated > 0 " +
			"      then (select description from treason_termination x where x.teamid=a.recruiter_teamid and x.id=reasonterminated) " +
			...
			" case when a.datehired is not null and a.contract=1 and fee_type=1 then fee when a.datehired is not null and contract=1 and fee_type=0 then yearly*fee/100 else null end fee, " +
			" notes " +
			columnsToShow.toString() + " from tinterviewschedule a " +
			" where a.id in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) and a.recruiter_teamid=? ";
	...
	public static String newUpdatedCompanies(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			" select a.id companyid, a.name companyname, dateentered datecreated, a.RECRUITERID USERID, editdate dateupdated, userfield_updatedate dateuserfieldupdated, " +
			" (select firstname||' '||lastname from tcustomer_company_owners b, trecruiter c" +
			" where b.companyid = a.id and b.teamid=a.teamid and b.isprimaryowner=1 and c.id = b.recruiterid) primaryowner," +
			" companytype, c.address1, c.address2, c.city, c.state, c.zipcode, c.countryid, c.phone, c.fax, c.url, c.email " +
			columnsToShow.toString() +
			" from " +
			" (select id, substr(max(sys_connect_by_path(typename, ',')),2) companytype " +
			" from " +
			" (select a.id, c.typename, rank() over (partition by a.id order by b.typeid) rk from tcustomercompany a, tcustomer_company_type b, tcustomer_company_types c " +
			" where a.teamid=? and (a.dateentered between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			" or a.editdate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			" or a.userfield_updatedate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss'))" +
			" and b.companyid(+)=a.id and b.teamid(+)=a.teamid and c.id(+)=b.typeid and c.teamid(+)=b.teamid) " +
			" start with rk=1 connect by prior rk+1 = rk group by id) b, tcustomercompany a, tcustomercompanyaddresses c" +
			" where a.id = b.id and c.companyid = a.id and c.default_address = 1";
	...
	public static String newUpdatedOpportunities(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			" select o.id, o.name, o.creatorid, o.createddate datecreated, o.startdate, o.enddate_est estimatedenddate, o.closedate, o.clientid, " +
			"   (select s.name from topportunity_stage s where s.teamid=o.teamid and s.id=o.status) stage, " +
			"   (select p.name from topportunity_priority p where p.teamid=o.teamid and p.id=o.priority) priority, " +
			"   reference, revenue, progress, laststatuschange, revenueduration, discontinue, discontinue_date " +
			" from topportunity o " +
	...
	public static String newUpdatedContacts(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			" select a.id contactid, a.firstname, a.lastname, timeentered datecreated, a.RECRUITERID USERID, editdate dateupdated, userfield_updatedate dateuserfieldupdated, a.companyid, a.companyname, a.departmentname department, a.title, a.active, " +
			"   a.email, a.workphone, a.workphoneext, a.homephone, a.cellphone, a.contactfax fax, " +
			"   b.address1, b.address2, b.city, b.state, b.zipcode, b.countryid " +
			columnsToShow.toString() +
			" from tcustomer a, tcustomeraddress b " +
			" where a.teamid=? and (a.timeentered between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"    or a.editdate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"    or a.userfield_updatedate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) " +
			"   and b.contactid=a.id and b.default_address=1 ";
	...
	public static String newUpdatedCompanyNotes(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select a.noteid, a.recruiterid userid, a.datecreated createdate, a.companyid, " +
			" substr(note,1,100) notefirst100chars " +
			" from tcustomercompanynotes a " +
	...
	public static String newUpdatedContactNotes(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			...
			"     or a.datemodified between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) " +
			"   and nvl(a.deleted,0) = 0 and a.type <> 6 " +
	...
	public static String deletedContactNotes(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select a.noteid, a.recruiterid userid, a.datedeleted " +
			" from tcustomernotes a, trecruiter b " +
			" where a.recruiter_teamid =? and a.datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
	...
	public static String newUpdatedJobs(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			" select /*+ no_merge(a) index_join(a IDX_TRFQ_1 IDX_TRFQ_UPDATE) */ " +
			"   a.id jobid, rfqno_team jobdivano, rfqrefno optionalreferenceno, divisionid, b.name divisionname, " +
			" 	pr.recruiterid primaryrecruiterid, ps.recruiterid primarysalesid, " +
			" 	a.companyid, a.RECRUITERID USERID, c.name companyname, " +
	...
	public static String newUpdatedJobUsers(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			" select a.jobid, a.dateupdated, b.recruiterid userid, c.firstname, c.lastname, " +
			"   sales, lead_sales primarysales, decode(sales+lead_sales,0,1,recruiter) recruiter, lead_recruiter primaryrecruiter, datelastassigned " +
			" from " +
			" (select jobid, teamid, max(dateupdated) dateupdated " +
			" from tjob_userupdated " +
			" where teamid =?  and dateupdated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			" group by jobid, teamid) a, trecruiterrfq b, trecruiter c " +
			" where b.rfqid=a.jobid and b.teamid=a.teamid and c.id=b.recruiterid ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String openJobsListByCompany(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			" select a.id jobid, rfqno_team jobdivano, rfqrefno optionalreferenceno, divisionid, b.name divisionname, " +
			" 	pr.recruiterid primaryrecruiterid, ps.recruiterid primarysalesid, " +
			" 	a.companyid, (select c.name from tcustomercompany c where c.id=a.companyid) companyname, " +
			" 	(select recruiterid from tcustomer_company_owners x where x.companyid=a.companyid and x.isprimaryowner=1) primaryownerid, " +
			...
			" 	nvl((select x.name from tcurrency x where x.id=payrate_currency), " +
			"   	nvl((select x.name from tcurrency x, tteam_currency y where y.teamid=a.teamid and y.defaultcurrency=1 and y.currencyid=x.id), 'USD')) currency " +
			" from trfq a, tdivision b, trecruiterrfq pr, trecruiterrfq ps " +
			" where a.teamid =? and a.companyid=? and a.jobstatus = 0 and a.divisionid = b.id(+) " +
			" and a.id = pr.rfqid(+) and 1 = pr.lead_recruiter(+) " +
			" and a.id = ps.rfqid(+) and 1 = ps.lead_sales(+) ";
		param.add(clientID);
		param.add(new Long(params[0]));
	...
	public static String openJobsListByContact(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			...
			"   	nvl((select x.name from tcurrency x, tteam_currency y where y.teamid=a.teamid and y.defaultcurrency=1 and y.currencyid=x.id), 'USD')) currency " +
			" from trfq a, tdivision b, trecruiterrfq pr, trecruiterrfq ps " +
			" where a.teamid =? and a.customerid=? and a.jobstatus = 0 and a.divisionid = b.id(+) " +
			" and a.id = pr.rfqid(+) and 1 = pr.lead_recruiter(+) " +
			" and a.id = ps.rfqid(+) and 1 = ps.lead_sales(+) ";
		param.add(clientID);
		param.add(new Long(params[0]));
	...
	public static String newUpdatedTasks(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			...
			"	decode(private,1,'Yes','No') private, datecreated, lastmodified " +
			" from tevent " +
			" where teamid=? and optional in (2,4) and status_new='ACTIVE' " +
	...
	public static String newUpdatedEvents(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select id, recruiterid, (select contactid from tevent_contacts where eventid=id and rownum=1) customerid, " +
			...
			"	decode(private,1,'Yes','No') private, datecreated, lastmodified, eventdate, timezone, duration, dtend enddate, " +
			"   rrule, rdate, exrule, exdate " +
			" from tevent " +
			" where teamid=? and optional in (0,1,3) and status_new='ACTIVE' " +
			"   and (datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"   or lastmodified between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) ";
		param.add(clientID);
		param.add(fromDate);
	...
	public static String deletedTasks(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select id, lastmodified datedeleted " +
			" from tevent " +
			" where teamid=? and optional in (2,4) and status_new='CANCELLED' " +
	...
	public static String newUpdatedCandidates(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			" select /*+ no_merge(a) index_join(a IDX_TCANDIDATE_DATERECEIVED IDX_TCANDIDATE_MANUAL IDX_TCAND_UFD_UPDATE) */ " +
			...
			" decode(bitand(nvl(privacyapplied,0), 4), 0, city, b.text) city, decode(bitand(nvl(privacyapplied,0), 4), 0, state, b.text) state, decode(bitand(nvl(privacyapplied,0), 4), 0, zipcode, b.text) zipcode," +
			" decode(bitand(nvl(privacyapplied,0), 8), 0, workphone, b.text) workphone, decode(bitand(nvl(privacyapplied,0), 8), 0, workphone_ext, b.text) workphone_ext, "+
			" decode(bitand(nvl(privacyapplied,0), 8), 0, homephone, b.text) homephone,  decode(bitand(nvl(privacyapplied,0), 8), 0, cellphone, b.text) cellphone," +
			" decode(bitand(nvl(privacyapplied,0), 8), 0, fax, b.text) fax " +
			columnsToShow.toString() +
			" from tcandidate a, (select text from (select text from gdpr_inaccessibility_text where teamid = ? or teamid = 0 order by teamid desc) where rownum = 1) b" +
...
//		System.out.println("BIData sql =" + sql);
	...
	public static String newUpdatedCandidateHR(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "select a.candidateid, a.legal_firstname, a.legal_middlename, a.legal_lastname, a.generationsuffix, a.ssn, to_char(a.dateofbirth, 'mm/dd/yyyy') as dateofbirth,"
				...
				+ " (select max(date_created) from tcandidate_hr_management b where b.teamid = a.teamid and candidateid = a.candidateid) dateupdated,"
				+ " a.userfield_updatedate dateuserfieldupdated"
				+ " from tcandidate_hr a, tcandidate c "
				+ " where a.teamid = ? and a.candidateid in (select candidateid from tcandidate_hr_management where teamid = ? and date_created between "
				...
				+ " where a.teamid = ? and a.candidateid in (select candidateid from tcandidate_hr_management where teamid = ? "
				+ " and date_created between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss'))"
				+ " and a.candidateid = c.id and a.teamid = c.teamid and nvl(c.PRIVACYAPPLIED,0) > 0";
		param.add(clientID);
		param.add(clientID);
		param.add(fromDate);
	...
	public static String newUpdatedEmployees(Long clientID, String  fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select a.candidateid candidateid, decode(dcatid, 1, 'Current', 2, 'Past', 3, 'Direct Placement') status, " + 
			" datecreated DATE_MODIFIED" +
			" from tcandidate_category a " +
			" where a.teamid = ? and catid=1 and a.datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')" +
			"   and dirty <> 2 ";
	...
	public static String newUpdatedActivities(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, Map<String, String> colNameToAliasMap) {
		...
		String sql =
			" select a.id activityid, a.datecreated, a.dateupdated, a.userfield_updatedate dateuserfieldupdated, " +
			" a.recruiterid userid, a.RECRUITERID_CREATOR UPDATEDBY_USERID, a.primarysalesid, a.candidateid, " +
			" 	a.rfqid jobid, decode(a.customerid,0,'',a.customerid) contactid, roleid, b.companyid, " +
			" case when a.id = (select min(x.id) from tinterviewschedule x " +
			"                   where x.candidateid=a.candidateid and x.rfqid=a.rfqid and nvl(x.roleid,0) < 900)" +
			"      then 1 else 0 end submittalflag,  " +
			" case when daterejected is not null or extdaterejected is not null then 1 else 0 end rejectflag, " +
			" case when a.dateinterview is not null then 1 else 0 end interviewflag, " +
			" case when a.datehired is not null then 1 else 0 end hireflag, " +
			" case when a.id = (select min(x.id) from tinterviewschedule x " +
			"                   where x.candidateid=a.candidateid and x.rfqid=a.rfqid and nvl(x.roleid,0) < 900) " +
			"      then a.datepresented else null end submittaldate, " +
			" case when roleid > 900 then daterejected else extdaterejected end rejectdate, " +
			" case when EXTREJECTREASONID is not null and EXTREJECTREASONID > 0 " +
			"      then (select reason from TREJECTION_REASONS rejreason where rejreason.teamid=a.recruiter_teamid and rejreason.id=a.EXTREJECTREASONID) " +
			"      else null end rejectreason, " +
			" a.dateinterview interviewdate, interview_timezoneid, a.datehired startdate, a.date_ended enddate, a.dateterminated terminationdate, " +
			" case when a.reasonterminated > 0 " +
			"      then (select description from treason_termination x where x.teamid=a.recruiter_teamid and x.id=a.reasonterminated) " +
			"      else null end terminationreason, " +
			" nvl((select x.name from trfq_position_types x where x.teamid=a.recruiter_teamid and x.id=a.contract), (select x.name from trfq_position_types x where x.teamid=0 and x.id=a.contract)) positiontype, " +
			" case when a.datehired is not null then a.pay_hourly else null end agreedbillrate, " +
			" case when a.datehired is not null and a.finalbillrateunit is not null " +
			"      then decode(a.finalbillrateunit,'h','Hourly','d','Daily','y','Yearly', " +
			"           (select x.name from trateunits x where x.teamid=a.recruiter_teamid and x.ratetype=1 and x.unitid=a.finalbillrateunit)) " +
			"      else null end billfrequency, " +
			" case when a.datehired is not null then decode(contract,1,a.yearly,a.hourly) else null end agreedpayrate, " +
			" case when a.datehired is not null then decode(contract,1,a.yearly_corporate,a.hourly_corporate) else null end corptocorp, " +
			" case when a.datehired is not null and a.payrateunits is not null " +
			...
			" case when a.datehired is not null " +
			"      then nvl((select x.name from tcurrency x where x.id=a.hourly_currency), " +
			...
			"      when a.datehired is not null and contract=1 and fee_type=0 then yearly*(case when fee < 0 then 0 else fee end)/100 else null end fee " +
			columnsToShow.toString() +
			" from tinterviewschedule a, tcustomer b " +
			" where a.recruiter_teamid = ? and (a.datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
	...
	public static String newUpdatedSIHActivitieslist(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql =
			" select a.id activityid, a.datecreated, a.dateupdated, a.userfield_updatedate dateuserfieldupdated, a.placementdate, a.interviewscheduledate, " +
			"   a.recruiterid userid, f.firstname userfirstname, f.lastname userlastname, " +
			"   a.primarysalesid, g.firstname primarysalesfirstname, g.lastname primarysaleslastname, " +
			...
			"   decode(a.customerid,0,'',a.customerid) contactid, d.firstname contactfirstname, d.lastname contactlastname," +
			" 	e.id companyid, e.name companyname, " +
			" case when a.roleid > 900 then 1 else 0 end internalsubmittalflag, " +
			" case when a.id = (select min(x.id) from tinterviewschedule x " +
			"                   where x.candidateid=a.candidateid and x.rfqid=a.rfqid and nvl(x.roleid,0) < 900)" +
			"      then 1 else 0 end externalsubmittalflag,  " +
			" case when daterejected is not null or extdaterejected is not null then 1 else 0 end rejectflag, " +
			" case when a.dateinterview is not null then 1 else 0 end interviewflag, " +
			" case when a.datehired is not null then 1 else 0 end hireflag, " +
			" case when a.roleid > 900 or a.id = (select min(x.id) from tinterviewschedule x " +
			"                   where x.candidateid=a.candidateid and x.rfqid=a.rfqid and nvl(x.roleid,0) < 900) " +
			"      then a.datepresented else null end submittaldate, " +
			" case when a.roleid > 900 then a.daterejected else null end internalrejectdate, " +
			" case when nvl(a.roleid,0) < 900 then a.extdaterejected else null end externalrejectdate, " +
			" case when EXTREJECTREASONID is not null and EXTREJECTREASONID > 0 " +
			"      then (select reason from TREJECTION_REASONS rejreason where rejreason.teamid=a.recruiter_teamid and rejreason.id=a.EXTREJECTREASONID) " +
			"      else null end rejectreason, " +
			" a.dateinterview interviewdate, interview_timezoneid, a.datehired startdate, a.date_ended enddate, a.dateterminated terminationdate, " +
			...
			" (case when a.hourly > 0 then a.hourly_corporate when a.daily > 0 then a.daily_corporate else a.yearly_corporate end) corptocorp, " +
			" case when a.finalbillrateunit is not null " + 
			"    then decode(a.finalbillrateunit,'h','Hourly','d','Daily','y','Yearly', " + 
			"    (select x.name from trateunits x where x.teamid=a.recruiter_teamid and x.ratetype=1 and x.unitid=a.finalbillrateunit)) " + 
			"    else null end billfrequency, " + 
			" (CASE WHEN a.HOURLY>0 THEN a.HOURLY WHEN a.DAILY>0 THEN a.DAILY WHEN a.YEARLY>0 THEN a.YEARLY ELSE 0 END) agreedpayrate, " + 
			" case when a.payrateunits is not null " + 
			"    then decode(case when daily > 0 then substr(payrateunits,2,1) when yearly > 0 then substr(payrateunits,3,1) else substr(payrateunits,1,1) end,'h','Hourly','d','Daily','y','Yearly', " + 
			"    (select x.name from trateunits x where x.teamid=a.recruiter_teamid and x.ratetype=0 and x.unitid=(case when daily > 0 then substr(payrateunits,2,1) when yearly > 0 then substr(payrateunits,3,1) else substr(payrateunits,1,1) end))) " + 
			"    else null end payfrequency, " + 
			" nvl((select x.name from tcurrency x where x.id=a.hourly_currency), " + 
			...
			"   or a.dateupdated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"   or a.userfield_updatedate between  to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) " +
			"   and a.candidateid = b.id and b.teamid=a.recruiter_teamid and a.rfqid = c.id and a.customerid = d.id(+) and c.companyid = e.id(+) " +
			"   and a.recruiterid = f.id and a.primarysalesid = g.id(+) ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
		param.add(fromDate);
	...
	public static String newHires(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select id placementid from tinterviewschedule where recruiter_teamid=? and placementdate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') and datehired is not null ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String updatedHires(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select id placementid from tinterviewschedule where recruiter_teamid=? and dateupdated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') and datehired is not null ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String newUpdatedCandidateNotes(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select a.noteid, " +
			" 	decode(a.type,1,'Last Attempt',2,'Last Reached',3,'Last Meeting',4,'Attribute',5,'Qualification'," +
			" 			6,'References Checked',7,'Incoming Call',8,'Outgoing Call',b.name) actiontype, " +
			" 	a.recruiterid userid, a.datecreated createdate, a.dateaction actiondate, " +
	...
	public static String deletedCandidates(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
				" select candidateid, datedeleted " +
				" from tcandidate_deleted a " +
				" where a.teamid=? and a.datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
			param.add(clientID);
			param.add(fromDate);
			param.add(toDate);
	...
	public static String deletedCandidateNotes(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select a.noteid, a.datedeleted " +
			" from tcandidatenotes a " +
			" where a.recruiter_teamid =? and a.datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String deletedCompanies(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select companyid, datedeleted " +
			" from tcustomercompany_deleted " +
			" where teamid=? and datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String deletedContacts(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select customerid contactid, datedeleted " +
	...
	public static String deletedEmployees(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select a.candidateid, a.datedeleted " +
			" from tcandidatequal_deleted a where a.teamid = ? " +
			" and a.datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') and a.catid=1 ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String deletedJobs(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
				" select rfqid jobid, datedeleted " +
				" from trfq_deleted " +
				" where teamid=? and datedeleted between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
		param.add(clientID);
		param.add(fromDate);
	...
	public static String mergedCompanies(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select companyid, mergetocompanyid, recruiterid userid, datemerged " +
			" from tcompany_merge " +
			" where teamid=? and datemerged between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String mergedContacts(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select contactid, mergetocontactid, recruiterid userid, datemerged " +
	...
	public static String mergedJobs(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select rfqid jobid, mergetorfqid mergetojobid, recruiterid userid, datemerged " +
			" from trfq_merge " +
			" where teamid = ? and datemerged between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
		param.add(clientID);
		param.add(fromDate);
	...
	public static String mergedCandidates(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
				" select distinct candidateid_gone candidateid, candidateid_new mergedtocandidateid, replace(to_char(datecreated, 'yyyy-mm-dd hh24:mi:ss'), ' ', 'T')||'.0' datemerged " + 
				" from tworkexp_stuff a " + 
				" where a.teamid=? and a.datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " + 
	...
	public static String candidateApplicationRecords(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			...
			"    where h.candidateid=a.candidateid and h.docid=a.docid and h.teamid=a.teamid and h.resumesourceflag=1 and f.id(+)=h.resumesourcerefid) referrer " +
			" from tcandidate_applyforjob a, tcandidate c, trejection r, (select text from (select text from gdpr_inaccessibility_text where teamid = ? or teamid = 0 order by teamid desc) where rownum = 1) g " +
			" where a.teamid = ? and a.dateapplied between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') and a.dirty = 0 " +
			"   and c.id=a.candidateid and c.teamid=a.teamid " +
			"   and r.candidateid(+)=a.candidateid and r.recruiter_teamid(+)=a.teamid and r.rfqid(+)=a.rfqid ";
		param.add(clientID);
		param.add(clientID);
	...
	public static String newApprovedBillingRecords(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			" select employeeid, recid, datecreated_real datecreated " +
			" from temployee_billingrecord " +
			" where recruiter_teamid=? and datecreated_real between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
	...
	public static String newApprovedSalaryRecords(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			" select employeeid, recid, datecreated_real datecreated " +
			" from temployee_salaryrecord a " +
			" where recruiter_teamid=? and datecreated_real between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			" and approved=1 and (closed is null or closed = 0) " +
			(restriction!=null && restriction.length>0 && restriction[0]!=null?" and exists (select 1 from temployee_billingrecord b where b.employeeid=a.employeeid and b.recruiter_teamid=a.recruiter_teamid and b.interviewid=a.interviewid and b.division=?) ":"");
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
		if(restriction!=null && restriction.length>0 && restriction[0]!=null) param.add(new Long(restriction[0]));
	...
	public static String updatedApprovedBillingRecords(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			" select employeeid, recid, datecreated_real datecreated, datecreated dateupdated " +
			" from temployee_billingrecord " +
	...
	public static String newUpdatedBillingRecords(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			" select a.EMPLOYEEID,a.RECID,a.RFQID JOBID,a.INTERVIEWID ACTIVITYID,nvl(a.APPROVED, 0) ARRPOVED,nvl(a.CLOSED, 0) DELETED,a.START_DATE,a.END_DATE,a.ACTUALSTART,a.ACTUALEND, " +
			...
			"   a.BILLING_CONTACT,a.HIRING_MANAGER,a.CUSTOMER_REFNO,a.JOBDIVA_REFNO,a.FREQUENCY,a.WEEK_ENDING, " +
	...
	public static String newUpdatedSalaryRecords(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =        		
			" select EMPLOYEEID,RECID,RFQID JOBID,INTERVIEWID ACTIVITYID,EFFECTIVE_DATE,END_DATE,nvl(APPROVED,0) APPROVED,nvl(CLOSED, 0) DELETED, " +
	...
	public static String billingRecordDetail(Long clientID, String[] params, Vector<Object> param, String[] restriction) {
		String sql =
			" select a.EMPLOYEEID,a.RECID,a.RFQID JOBID,a.INTERVIEWID ACTIVITYID,a.APPROVED,a.CLOSED,a.START_DATE,a.END_DATE,a.ACTUALSTART,a.ACTUALEND, " +
			"   a.DATECREATED_REAL DATECREATED,a.DATECREATED DATEUPDATED,a.CREATED_BY,a.APPROVERID, " +
			"   a.PRIMARY_SALESPERSON,a.SECONDARY_SALESPERSON,a.TERTIARY_SALESPERSON,a.PRIMARY_RECRUITER,a.SECONDARY_RECRUITER,a.TERTIARY_RECRUITER, " +
			"   a.BILLING_CONTACT,a.HIRING_MANAGER,a.CUSTOMER_REFNO,a.JOBDIVA_REFNO,a.FREQUENCY,a.WEEK_ENDING, " +
			"   a.BILL_RATE,a.BILL_RATE_PER,a.OVERTIME_RATE1,a.OVERTIME_RATE1_PER,a.OVERTIME_RATE2,a.OVERTIME_RATE2_PER,a.HOURS_PER_DAY, " +
	...
	public static String employeeBillingRecordsDetail(Long clientID, String[] params, Vector<Object> param, String[] restriction) {
		String sql =
			...
			"   a.PAYMENTTERMS,a.EXPENSEENABLED,a.POID,b.AMOUNT POAMOUNT,b.START_DATE PO_START_DATE,b.END_DATE PO_END_DATE,a.OVERTIMEEXEMPT,a.DIVISION,a.TIMESHEET_ENTRY_FORMAT,a.CUSTOMER_REFNO_SUB,a.TIMESHEET_INSTRUCTION " +
			" from temployee_billingrecord a, tpo_setting b " +
			" where employeeid=? and recruiter_teamid=? and b.teamid(+)=a.recruiter_teamid and b.id(+)=a.poid " +
				(restriction!=null && restriction.length > 0 && restriction[0]!=null? " and a.division=? ":"") +
			" order by recid ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String salaryRecordDetail(Long clientID, String[] params, Vector<Object> param, String[] restriction) {
		String sql =
			" select EMPLOYEEID,RECID,RFQID JOBID,INTERVIEWID ACTIVITYID,EFFECTIVE_DATE,END_DATE,APPROVED,CLOSED, " +
			"   DATECREATED_REAL DATECREATED,DATECREATED DATEUPDATED,CREATED_BY, " +
			"   STATUS,PER_DIEM,PER_DIEM_PER,OUTSIDE_COMMISSION,OUTSIDE_COMMISSION_PER,OTHER_EXPENSES,OTHER_EXPENSES_PER, " +
			"   INSURED,SALARY,SALARY_PER,OVERTIME_RATE1,OVERTIME_RATE1_PER,OVERTIME_RATE2,OVERTIME_RATE2_PER,OVERTIMEEXEMPT, " +
			"   SUBCONTRACT_COMPANYID,SUBCONTRACT_PAYONREMIT," +
	...
	public static String salaryRecordsDetail(Long clientID, String[] params, Vector<Object> param, String[] restriction) {
		String sql =
			...
			"   SUBCONTRACT_COMPANYID,SUBCONTRACT_PAYONREMIT," +
			"   OH_W2,OH_INSURANCE,OH_VACATION,OH_CORP_INSURANCE,OH_401K,OH_C2C,OH_PERDIEM, " +
			"   TAXID,PAYMENTTERMS,BIRTHDAY,VISA_STATUS,SSN,VACATION,COMMENTS,ADP_FILE_NO,ADPCOCODE ADP_CO_CODE " +
			" from temployee_salaryrecord a " +
			" where employeeid in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual )) and recruiter_teamid=? " +
			(restriction!=null && restriction.length>0 && restriction[0]!=null?" and exists (select 1 from temployee_billingrecord b where b.employeeid=a.employeeid and b.recruiter_teamid=a.recruiter_teamid and b.interviewid=a.interviewid and b.division=?) ":"");
		StringBuilder candidates = new StringBuilder();
	...
	public static String newInvoices(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			" select invoiceid, customer_refno po_no, customer_refno_sub customer_referenceno, billingid billing_recid, rfqid jobid, " +
			"   decode(invoicetype,1,'regular',3,'whole project',4,'expense',5,'perm placement','other') invoicetype, " +
			"   fromdate, todate, employeeid, billing_contact, invoicedate, hoursworked, amount  " +
			" from temployee_invoice " +
			" where recruiter_teamid=? and datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"   and approved=1 and (void is null or void = 0) and (closed is null or closed = 0) ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String voidedInvoices(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql =
			...
			"   fromdate, todate, employeeid, billing_contact, invoicedate, hoursworked, amount " +
			" from temployee_invoice " +
			" where recruiter_teamid=? and void = 1 " +
			"   and datevoided between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String invoiceDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select invoiceid, customer_refno po_no, customer_refno_sub customer_referenceno, billingid billing_recid, rfqid jobid, " +
			"   decode(invoicetype,1,'regular',3,'whole project',4,'expense',5,'perm placement','other') invoicetype, " +
			"   fromdate, todate, employeeid, billing_contact, invoicedate, hoursworked, " +
			"   amount-nvl((select sum(b.expense) from tinvoice_expense b where b.invoiceid=a.invoiceid and b.recruiter_teamid=a.recruiter_teamid),0) amount, " +
			"   to_char(itemdescription) itemdescription, to_char(comments) comments " +
			" from temployee_invoice a " +
			" where invoiceid=? and recruiter_teamid=? " +
			"   union " +
			" select invoiceid, null po_no, null customer_referenceno, null billing_recid, null jobid, " +
			"   'overtime' invoicetype, null fromdate, null todate, null employeeid, null billing_contact, " +
			"   null invoicedate, quantity hoursworked, rate_per_unit*quantity amount, to_char(item) itemdescription, null comments " +
			" from temployee_invoice_ot1 " +
			" where invoiceid=? and recruiter_teamid=? " +
			"   union " +
			" select invoiceid, null po_no, null customer_referenceno, null billing_recid, null jobid, " +
			"   'expense' invoicetype, null fromdate, null todate, null employeeid, null billing_contact, " +
			"   null invoicedate, null hoursworked, expense amount, to_char(item) itemdescription, to_char(comments) comments " +
			" from tinvoice_expense " +
			" where invoiceid=? and recruiter_teamid=? and billable=1 ";
		param.add(new Long(params[0]));
		param.add(clientID);
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String placementDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			" select invoiceid, salary, fee, startdate, " +
			"   salespersonid, commision sales_comm, salespersonid2, sales_comm2, salespersonid3, sales_comm3, " +
			"   recruiterid, rec_comm, recruiterid2, rec_comm2, recruiterid3, rec_comm3 " +
			" from tplacements a " +
			" where invoiceid=? and recruiter_teamid=? ";
		param.add(new Long(params[0]));
		param.add(clientID);
	...
	public static String approvedTimesheets(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			"SELECT w.timecardid timesheetid, w.employeeid, w.billing_recid, w.salary_recid, w.weekendingdate, w.datecreated,  " + 
			...
			"where w.recruiter_teamid=? and w.approved_on between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')  " + 
			"and w.approved=1 " +
	...
	public static String newUpdatedApprovedTimesheets(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			"SELECT w.timecardid timesheetid, w.employeeid, w.billing_recid, w.salary_recid, w.weekendingdate, w.datecreated, " + 
			"w.approved_on dateapproved, w.approved_by userapproved, NVL(b.BILLRATE_NAME,'Regular Hours') HOURS_DESCRIPTION, " + 
			"w.hoursworked, w.reg_hours, w.ot_hours, w.dt_hours, w.comments, " + 
			"s.SALARY REG_PAY, s.OVERTIME_RATE1 OT_PAY, s.OVERTIME_RATE2 DT_PAY, s.SALARY_PER REG_PAY_RATE_UNIT, s.OVERTIME_RATE1_PER OT_PAY_RATE_UNIT, s.OVERTIME_RATE2_PER DT_PAY_RATE_UNIT,  " + 
			"b.BILL_RATE REG_BILL, b.OVERTIME_RATE1 OT_BILL, b.OVERTIME_RATE2 DT_BILL, b.BILL_RATE_PER REG_BILL_RATE_UNIT, b.OVERTIME_RATE1_PER OT_BILL_RATE_UNIT, b.OVERTIME_RATE1_PER DT_BILL_RATE_UNIT " + 
			"FROM temployee_wed w JOIN TEMPLOYEE_SALARYRECORD s ON w.RECRUITER_TEAMID = s.RECRUITER_TEAMID AND w.EMPLOYEEID = s.EMPLOYEEID AND w.BILLING_RECID = s.RECID " + 
			"JOIN TEMPLOYEE_BILLINGRECORD b ON w.RECRUITER_TEAMID = b.RECRUITER_TEAMID AND w.EMPLOYEEID = b.EMPLOYEEID AND w.BILLING_RECID = b.RECID " + 
			"WHERE w.recruiter_teamid=? and (w.datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')   " + 
			"or w.approved_on between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) " + 
			"and w.approved=1 " +
			(restriction!=null && restriction.length > 0 && restriction[0]!=null? " and b.division=? ":"");
		param.add(clientID);
		param.add(fromDate);
	...
	public static String newUpdatedTimesheets(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			...
			"s.SALARY_PER REG_PAY_RATE_UNIT, s.OVERTIME_RATE1_PER OT_PAY_RATE_UNIT, s.OVERTIME_RATE2_PER DT_PAY_RATE_UNIT, " + 
			"b.BILL_RATE REG_BILL, b.OVERTIME_RATE1 OT_BILL, b.OVERTIME_RATE2 DT_BILL, b.BILL_RATE_PER REG_BILL_RATE_UNIT, " + 
			"b.OVERTIME_RATE1_PER OT_BILL_RATE_UNIT, b.OVERTIME_RATE1_PER DT_BILL_RATE_UNIT, " + 
			"DECODE(w.APPROVED,'0','PENDING','1','APPROVED','2','REJECTED','') STATUS " + 
			"FROM temployee_wed w JOIN TEMPLOYEE_SALARYRECORD s ON w.RECRUITER_TEAMID = s.RECRUITER_TEAMID AND w.EMPLOYEEID = s.EMPLOYEEID AND w.BILLING_RECID = s.RECID " + 
			"JOIN TEMPLOYEE_BILLINGRECORD b ON w.RECRUITER_TEAMID = b.RECRUITER_TEAMID AND w.EMPLOYEEID = b.EMPLOYEEID AND w.BILLING_RECID = b.RECID " + 
			"WHERE w.recruiter_teamid=? and (w.datecreated between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " + 
			"or w.approved_on between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) " + 
	...
	public static String weekendingOnTimesheets(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql =
			"SELECT s.ADP_FILE_NO, " + 
			"(SELECT c.ID FROM TRFQ r JOIN TCUSTOMER c ON r.CUSTOMERID = c.ID AND r.TEAMID = c.TEAMID WHERE s.RFQID = r.ID AND s.RECRUITER_TEAMID = r.TEAMID) COMPANY_ID, " + 
			...
			"where w.recruiter_teamid=? and w.weekendingdate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')  " + 
			"and w.approved=1 " +
			(restriction!=null && restriction.length > 0 && restriction[0]!=null? " and b.division=? ":"");
		param.add(clientID);
		param.add(fromDate);
	...
	public static String timesheetDetail(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, String[] restriction) {
		...
		String sql =
			"select a.timecardid timesheetid, a.externalid, a.employeeid, a.billing_recid, s.RECID SALARY_RECID, a.weekendingdate, " + 
			...
			"LEFT JOIN TEMPLOYEE_SALARYRECORD s ON a.RECRUITER_TEAMID = s.RECRUITER_TEAMID AND a.EMPLOYEEID = s.EMPLOYEEID AND a.SALARY_RECID = s.RECID " + 
			"where a.recruiter_teamid=? " + 
			(externalid_flag? "and a.externalid=? " : "and (a.timecardid=? or a.externalid=?) ") +
			(restriction!=null && restriction.length>0 && restriction[0]!=null?" and exists (select 1 from temployee_billingrecord b where b.employeeid=a.employeeid and b.recruiter_teamid=a.recruiter_teamid and b.recid=a.billing_recid and b.division=?) ":"");
		param.add(clientID);
		if(externalid_flag) 
			param.add(params[0]);
		else {
	...
	public static String employeeTimesheetImageDetail(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, String[] restriction) {
		String sql =
			" select a.weekendingdate, b.image_filename filename, b.image_bytes filecontent " +
			" from temployee_wed_images a, temployee_timesheet_images b " +
			" where a.recruiter_teamid=? and a.weekendingdate between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
			"   and a.employeeid=? and a.billing_recid=? " +
	...
	public static String employeeTimesheetImageDetailByTimecardId(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param, String[] restriction) {
		String sql =
				" select a.weekendingdate, b.image_filename filename, b.image_bytes filecontent " +
				" from temployee_wed t, temployee_wed_images a, temployee_timesheet_images b " +
				" where t.recruiter_teamid=? and t.employeeid=? and t.timecardid=? " +
				"   and a.recruiter_teamid=t.recruiter_teamid and a.employeeid=t.employeeid and a.billing_recid=t.billing_recid and a.weekendingdate = t.weekendingdate" +
				"   and b.recruiter_teamid=a.recruiter_teamid and b.imageid=a.imageid " +
				(restriction!=null && restriction.length>0 && restriction[0]!=null?" and exists (select 1 from temployee_billingrecord br where br.employeeid=t.employeeid and br.recruiter_teamid=t.recruiter_teamid and br.recid=t.billing_recid and br.division=?) ":"");
		param.add(clientID);
		param.add(new Long(params[0]));
		param.add(new Long(params[1]));
		if(restriction!=null && restriction.length > 0 && restriction[0]!=null) param.add(new Long(restriction[0]));
	...
	public static String jobsDashboardByUser(Long clientID, String fromDate, String toDate, String[] params, Vector<Object> param) {
		String sql = " select b.id jobid, b.dateissued, b.rfqno_team jobdivano, b.rfqrefno refno, b.rfqtitle jobtitle, " +
				"   b.contract, b.city, b.state, (select d.name from tdivision d where d.teamid=a.teamid and d.id=b.divisionid) division, " +
				...
				"   (select nvl(sum(case when datehired is not null then 1 else 0 end),0) from tinterviewschedule t1 where t1.rfqid=a.rfqid) hires " +
				" from trecruiterrfq a, trfq b " +
	...
	public static String contactHotlistDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql = "select h.contactid, c.firstname, c.lastname, c.title, c.companyname, c.workphone, c.email " +
				"from tcontact_hotlist_contacts h, tcustomer c " +
				"where h.teamid=? and h.hotlist_id=? and nvl(deleted,0)=0 and c.id=h.contactid ";
		param.add(clientID);
		param.add(new Long(params[0]));
		return sql;
	}
	...
	public static String candidateHotlistDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql = " select candidateid, firstname, lastname, tag abstract, homephone, city, state, datereceived_original date_received, datecreated date_available " +
				"from tcandidate_hotlist " +
	...
	public static String userContactHotlistDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql = "select h.hotlist_id, h.contactid, c.firstname, c.lastname, c.title, c.companyname, c.workphone, c.email " +
				"from tcontact_hotlist_contacts h, tcustomer c " +
	...
	public static String userCandidateHotlistDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql = " select hotlist_id, candidateid, firstname, lastname, tag abstract, homephone, city, state, datereceived_original date_received, datecreated date_available " +
				"from tcandidate_hotlist " +
	...
	public static String contactHotlistsDetail(Long clientID, String[] params, Vector<Object> param) {
		...
		String sql = "select h.hotlist_id, h.contactid, c.firstname, c.lastname, c.title, c.companyname, c.workphone, c.email " +
				"from tcontact_hotlist_contacts h, tcustomer c " +
				"where h.teamid=? and h.hotlist_id in (Select * from THE (Select cast(sf_inlist(?) as sf_inlist_table_type ) from dual ))" +
				" and nvl(deleted,0)=0 and c.id=h.contactid order by h.hotlist_id";
		StringBuilder hotlists = new StringBuilder();
		for(int i = 0; i < params.length; i++) {
			if(i > 0) hotlists.append(",");
	...
	public static String submittalbyCandbyJob(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
				"select id, recruiterid userid, primarysalesid, candidateid, rfqid jobid, customerid contactid, roleid, datecreated, dateupdated, " +
				"  datepresented submittaldate, " +
				" case when roleid > 900 then daterejected else extdaterejected end rejectiondate, " +
				"  dateinterview, interview_timezoneid, datehired startdate, date_ended enddate, a.dateterminated terminationdate, " +
				"  managerfirstname, managerlastname, " +
	...
	public static String CandidateNotesListDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql =
			...
			" 	case when note_clob is null then substr(note,1,100) else to_char(substr(note_clob,1,100)) end notefirst100chars " +
			" from tcandidatenotes a, tactiontype_candidate b " +
			" where a.recruiter_teamid = ? and a.candidateid = ? " +
			" and (a.auto = 0 or a.auto = 3) and nvl(a.deleted,0) = 0 and a.type = b.id(+) and a.recruiter_teamid=b.teamid(+) ";
		param.add(clientID);
		param.add(new Long(params[0]));
		return sql;
	...
	public static String CandidateEmailRecords(Long clientID, String[] params, Vector<Object> param) {
		String sql =
				"select b.messageid, a.from_address, a.to_address, a.subject, a.dateofemail  " +
		        " from temails_saved a, temails_saved_links b " +
				" where b.link_personid = ? and b.teamid = ? and b.type = 2 and a.id = b.messageid ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
	}
	...
	public static String ContactEmailRecords(Long clientID, String[] params, Vector<Object> param) {
		String sql =
				"select b.messageid, a.from_address, a.to_address, a.subject, a.dateofemail  " +
		        " from temails_saved a, temails_saved_links b " +
				" where b.link_personid = ? and b.teamid = ? and b.type = 1 and a.id = b.messageid ";
		param.add(new Long(params[0]));
		param.add(clientID);
		return sql;
	}
	...
	public static String candidateResumeSubmittedtoJob(Long clientID,  String[] params, Vector<Object> param) {
		String sql =
				"select candidateid, rfqid jobid, recruiterid userid, datecreated, document, filename, ziprtf RESUME_BASE64ENCODED" +
				" from tpresenteddocument where teamid =? and candidateid = ? and rfqid=?";
			param.add(clientID);
			param.add(new Long(params[0]));
			param.add(new Long(params[1]));
			return sql;
	...
	public static String savedEmails(Long clientID,  String fromDate, String toDate, Vector<Object> param) {
		String sql =
				"select a.messageid, a.datecreated, b.from_address, b.to_address, b.subject, b.dateofemail, d.recruiterid userid, case when c.type=1 then 'Contact' else 'Candidate' end PERSONTYPE, c.link_personid" +
				" from temails_saved_date a, temails_saved b, temails_saved_links c, temails_saved_recruiters d" +
				" where a.teamid=? and a.datecreated between str_to_date(?,'%m/%d/%Y %H:%i:%s') and str_to_date(?,'%m/%d/%Y %H:%i:%s')" +
	...
	public static String submittalsInterviewsHiresByRecruiter(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT ID, FIRSTNAME||' '||LASTNAME RECRUITERNAME,  " +
				"NVL(s.VALUE,0) AS SUBMITTALS, NVL(i.VALUE,0) AS INTERVIEWS, NVL(h.VALUE,0) AS HIRES " +
				"FROM TRECRUITER r  " +
				"LEFT JOIN ( SELECT RECRUITERID, count(distinct (rfqid||'~'||candidateid)) VALUE  " +
				"            FROM tinterviewschedule WHERE RECRUITER_TEAMID = ? AND datecreated between  " +
				"            to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " +
				"            group by RECRUITERID) s ON r.ID = s.RECRUITERID  " +
				"LEFT JOIN ( SELECT RECRUITERID, count(distinct (rfqid||'~'||candidateid)) VALUE  " +
				"            FROM tinterviewschedule where RECRUITER_TEAMID = ? AND (INTERVIEWSCHEDULEDATE between " +
				"            to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')) and dateinterview is not null  " +
				"            group by RECRUITERID) i ON r.ID = i.RECRUITERID " +
				"LEFT JOIN ( SELECT RECRUITERID, COUNT(*) VALUE  " +
				"            FROM tinterviewschedule where RECRUITER_TEAMID = ? and placementdate between  " +
				"            to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') and datehired is not null  " +
				"            group by RECRUITERID) h ON r.ID = h.RECRUITERID " +
				"WHERE r.GROUPID = ? AND (s.VALUE > 0 OR i.VALUE > 0 OR h.VALUE > 0) " +
				"ORDER BY ID";
	...
	public static String resumeCountByRecruiter(Long clientID, String fromDate, String toDate, String[] params,
			Vector<Object> param) {
		String sql = "SELECT RECRUITERID ID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = t.RECRUITERID) RECRUITERNAME, COUNT(*) VALUE  " +
				"FROM TCANDIDATEDOCUMENT_HEADER t WHERE TEAMID = ? AND DATERECEIVED BETWEEN  " +
				"to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')  " +
	...
	public static String paychexProfiles(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = 
				"SELECT p.EMPLOYEEID_PAYCHECK, " + 
				"(SELECT c.ID FROM TRFQ r JOIN TCUSTOMER c ON r.CUSTOMERID = c.ID AND r.TEAMID=c.TEAMID WHERE r.ID = s.RFQID AND r.TEAMID=s.RECRUITER_TEAMID) COMPANY_ID, " + 
				...
				"'6 - Resume Contract (Ind Contractor)') REASON, p.STATUSDATE, p.POSITION,  p.EMPLOYEETYPE,  " + 
				"(SELECT c.NAME FROM TCUSTOMERCOMPANY c WHERE c.id = s.subcontract_companyID and c.teamid = s.recruiter_teamid) IC_COMPANY_NAME,  " + 
				"p.HOMEEMAIL, p.WORKEMAIL, p.HIREDATE, p.JOBNUMBER, p.WORKSTATE, " + 
				"DECODE(p.STATETAXABILITY, 'Y','Y - Withhold','N','N - Taxable') STATETAXABILITY, p.STATEFILLINGSTATUS, p.STATEUNEMPLOYMENT, p.STATEINCOMETAX,    " + 
				"DECODE(p.STATEDISABILITY, 'Y','Y - Withhold','N','N - Taxable') STATEDISABILITY, p.FEDERALIDNUMBER,    " + 
				"DECODE(p.FEDERALFILINGSTATUS,'S','S - Single','M','M - Married','MWS','Married but withhold at a single rate') FEDERALFILINGSTATUS,    " + 
				"p.FEDERALALLOWANCES, p.FEDERALADDITIONALTAXAMOUNT,    " + 
				"DECODE(s.SALARY_PER,'H','HR - Hourly Rate','D','DR - Daily Rate','Y','AS - Annual Salary') PAY_RATE_1, s.SALARY PAY_RATE_AMOUNT_1,    " + 
				"DECODE(p.OVERTIMEEXEMPT, 'Y','Y - Withhold','N','N - Taxable') OVERTIMEEXEMPT,    " + 
				...
				"JOIN TEMPLOYEE_SALARYRECORD s ON p.RECRUITER_TEAMID = s.RECRUITER_TEAMID AND p.EMPLOYEEID = s.EMPLOYEEID and p.SALARY_RECID = s.RECID   " + 
				"WHERE p.RECRUITER_TEAMID = ? AND s.approved=1 and nvl(s.closed,0) = 0   " + 
				"AND (p.DATECREATED BETWEEN to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')    " + 
				"OR p.DATEUPDATED BETWEEN to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss'))   " + 
	...
	public static String adpProfiles(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql = 
				" select a.employeeid, h.legal_firstname, h.legal_middlename, h.legal_lastname, h.generationsuffix generation_suffix, a.hirereason reason_for_hire, " + 
				"   a.companycode company_code, h.ssn, " + 
				...
				"   to_char(h.dateofbirth, 'mm/dd/yyyy') date_of_birth, a.country, a.address1, a.address2, a.address3, a.city, a.county, a.state, a.zipcode,   " + 
				"   a.hphoneprefix || a.hphonelast telephone, a.email,  " + 
				...
				"   decode(LOWER(s.salary_per),'h','H - Hourly','s','S - Salary','d','D - Daily','n','N - None','') regular_pay_rate, s.salary regular_pay_amount,  " + 
				"   nvl(a.standardhours, 0) standard_hours, nvl(a.paygroup, '1') pay_group,   " + 
				...
				"   a.federalmarritalstatus) federal_marital_status, " +
				"   a.federalexemptions federal_exemptions, " +
				"   a.W4_MULTIPLEJOBS \"W-4_MULTIPLE_JOBS\", a.W4_DEPENDENTS \"W-4_DEPENDANTS\", a.W4_OTHERINCOME \"W-4_OTHER_INCOME\", " + 
				"   a.W4_DEDUCTIONS \"W-4_DEDUCTIONS\", " + 
				"   a.workedinstate worked_state_tax_code,   " + 
				...
				"   a.prenotificationmethod1 prenotification_method, a.bankdepositprenotedate1 bank_deposit_prenote_date, s.RECID SALARY_RECID, b.RECID BILLING_RECID  " + 
				" from temployee_adp a " + 
				...
				" left join tcandidate_hr h on h.teamid = a.recruiter_teamid and h.candidateid = a.employeeid  " + 
				" where a.recruiter_teamid = ? " + 
				...
				"   and not exists ( " + 
				"     select 1 from temployee_salaryrecord s1 where s1.employeeid=s.employeeid and s1.recruiter_teamid=s.recruiter_teamid  " + 
				"     and s1.interviewid=s.interviewid and s1.approved=1 and nvl(s1.closed,0)=0 and s1.recid > s.recid) " +
				"   and b.approved=1 and nvl(b.closed,0) = 0 " +
				"   and not exists ( " +
				"     select 1 from temployee_billingrecord b1 where b1.employeeid=b.employeeid and b1.recruiter_teamid=b.recruiter_teamid  " + 
				"     and b1.interviewid=b.interviewid and b1.approved=1 and nvl(b1.closed,0)=0 and b1.recid > b.recid) " +
				(restriction!=null && restriction.length>0 && restriction[0]!=null?" and b.division=? ":"");
	...
	public static String adpProfile(Long clientID, String[] params, Vector<Object> param, String[] restriction) {
		String sql = 
				...
				"nvl(a.standardhours, 0) standard_hours, nvl(a.paygroup, '1') pay_group, " + 
				"decode(LOWER(a.federalmarritalstatus),'d','Single or Married filing separately','j','Married filing jointly (or Qualifying widow(er))','h','Head of Household'," + 
				"a.federalmarritalstatus) federal_marital_status, a.federalexemptions federal_exemptions, " + 
				"a.W4_MULTIPLEJOBS \"W-4_MULTIPLE_JOBS\", a.W4_DEPENDENTS \"W-4_DEPENDANTS\", a.W4_OTHERINCOME \"W-4_OTHER_INCOME\", " + 
				"a.W4_DEDUCTIONS \"W-4_DEDUCTIONS\", " + 
				"a.workedinstate worked_state_tax_code, " + 
				"decode(LOWER(a.statemaritalstatus),'m','M - Married','n','N - Married - Head of Household','r','R - Single - HOH/ Qualifying Dependent','s','S - Single', " + 
				...
				"    join temployee_billingrecord b on b.recruiter_teamid = a.recruiter_teamid and b.employeeid = a.employeeid and b.interviewid = s.interviewid " + 
				"    left join tcandidate_hr h on h.teamid = a.recruiter_teamid and h.candidateid = a.employeeid " + 
				"where a.recruiter_teamid = ? AND a.EMPLOYEEID = ? " + 
				"and s.approved=1 and nvl(s.closed,0) = 0 " + 
				"and not exists ( " + 
				"  select 1 from temployee_salaryrecord s1 where s1.employeeid=s.employeeid and s1.recruiter_teamid=s.recruiter_teamid " + 
				"  and s1.interviewid=s.interviewid and s1.approved=1 and nvl(s1.closed,0)=0 and s1.recid > s.recid) " + 
	...
	public static String companyCountByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = t.RECRUITERID) USERNAME, COUNT(*) COUNT " + 
				"from TCUSTOMERCOMPANY t " + 
				"WHERE TEAMID = ? " + 
				"AND DATEENTERED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') " + 
				"GROUP BY RECRUITERID";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String jobsCountByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = a.RECRUITERID) USERNAME, COUNT(*) COUNT " + 
				"FROM TRFQ a " + 
				"WHERE TEAMID = ? AND " + 
				"(a.DATEISSUED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss')) " + 
				"GROUP BY RECRUITERID";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String contactCountByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = t.RECRUITERID) USERNAME, COUNT(*) COUNT " + 
				"FROM TCUSTOMER t " + 
				"WHERE TEAMID= ? AND " + 
				"(TIMEENTERED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss')) " + 
				"GROUP BY RECRUITERID ";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String candidateNoteCountByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = c.RECRUITERID) USERNAME, COUNT(*) COUNT " + 
				"FROM TCANDIDATENOTES c " + 
				"where RECRUITER_TEAMID = ? and recruiterid>0 and nvl(deleted,0) = 0 and (auto = 0 or auto = 3) " + 
				"AND DATECREATED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') " + 
	...
	public static String contactNoteCountByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = c.RECRUITERID) USERNAME, COUNT(*) COUNT " + 
				"FROM TCUSTOMERNOTES c " + 
				"WHERE RECRUITER_TEAMID = ? AND (DATEMODIFIED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss')) " + 
				"AND NVL(DELETED,0) = 0 and recruiterid>0 " + 
				"GROUP BY RECRUITERID order by count(*) desc";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String candidateNoteCountByActionByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = c.RECRUITERID) USERNAME, " + 
				"    DECODE(c.TYPE,1,'Last Attempt',2,'Last Reached',3,'Last Meeting',4,'Attribute',5,'Qualification',6,'References Checked', " + 
				"    7,'Incoming Call',8,'Outgoing Call',a.NAME) ACTIONTYPE, COUNT(*) COUNT " + 
				"FROM TCANDIDATENOTES c LEFT JOIN TACTIONTYPE_CANDIDATE a ON c.TYPE = a.ID AND c.RECRUITER_TEAMID = a.TEAMID " + 
				"WHERE c.RECRUITER_TEAMID = ? AND c.RECRUITERID>0 AND NVL(c.DELETED,0) = 0 AND (c.AUTO = 0 OR c.AUTO = 3) " + 
				"AND DATECREATED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') " + 
				"GROUP BY RECRUITERID, DECODE(c.TYPE,1,'Last Attempt',2,'Last Reached',3,'Last Meeting',4,'Attribute', " + 
				"    5,'Qualification',6,'References Checked',7,'Incoming Call',8,'Outgoing Call',a.NAME) " + 
				"ORDER BY USERNAME";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String contactNoteCountByActionByUser(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, (SELECT FIRSTNAME||' '||LASTNAME FROM TRECRUITER r WHERE r.ID = c.RECRUITERID) USERNAME, " + 
				"DECODE(c.TYPE,1,'Last Attempt',2,'Last Reached',3,'Last Meeting',4,'Attribute',5,'Qualification',6,'References Checked', " + 
				"7,'Incoming Call',8,'Outgoing Call',a.NAME) ACTIONTYPE, COUNT(NOTEID) COUNT " + 
				"FROM TCUSTOMERNOTES c LEFT JOIN TACTIONTYPE_CANDIDATE a ON c.TYPE = a.ID AND c.RECRUITER_TEAMID = a.TEAMID " + 
				"WHERE RECRUITER_TEAMID = ? AND NVL(c.DELETED,0) = 0 and recruiterid>0 " + 
				"AND (DATECREATED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') ) " + 
				"GROUP BY RECRUITERID, DECODE(c.TYPE,1,'Last Attempt',2,'Last Reached',3,'Last Meeting',4,'Attribute',5,'Qualification',6,'References Checked', " + 
				"7,'Incoming Call',8,'Outgoing Call',a.NAME) " + 
				"ORDER BY USERNAME";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String newResumesDownloaded(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT RECRUITERID USERID, candidateid, docid, dbid, global_id resumeid, datecreated_original datecreated, datecreated dateupdated, " + 
				"    datereceived_original datefirstdownloaded, datereceived datelastdownloaded " + 
				"from tcandidatedocument_header " + 
				"where teamid= ? " + 
				"and datereceived_original between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss')";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String newUpdatedCandidateAvailabilityRecords(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = "SELECT candidateid, case when flag=2 then 1 else 0 end unavailableindef, dateavailable, " +
				" DATE_CREATED datecreated " + 
				"FROM tcandidate_unreachable where teamid = ? " + 
				"and DATE_CREATED between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " + 
				"    union " + 
				"select candidateid, 0 unavailableindef, max(a.date_created) dateavailable, " + 
				"max(DATE_CREATED) datecreated " + 
				"FROM tcandidate_unreachable_archive a WHERE a.teamid = ? " + 
				"and DATE_CREATED between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " + 
				"GROUP BY CANDIDATEID " + 
				"    union " + 
				"select candidateid, 0 unavailableindef, max(h.datecreated) dateavailable, " + 
				"max(DATECREATED) datecreated " + 
				"FROM tcandidatedocument_header h where h.teamid = ? " + 
				"and DATECREATED between to_date(?,'mm/dd/yyyy hh24:mi:ss') and to_date(?,'mm/dd/yyyy hh24:mi:ss') " + 
				"and not exists (select 1 from tcandidatedocument_header d where d.candidateid=h.candidateid and d.teamid=h.teamid and d.daterefreshed > h.daterefreshed) " + 
				"and not exists (select 1 from tcandidate_unreachable x where x.candidateid=h.candidateid and x.teamid=h.teamid) " + 
				"and not exists (select 1 from tcandidate_unreachable_archive a where h.candidateid=a.candidateid and h.teamid=a.teamid) " + 
				"GROUP BY CANDIDATEID";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
	...
	public static String candidateAttributeDetail(Long clientID, String[] params, Vector<Object> param) {
		String sql = "SELECT CANDIDATEID, RECRUITERID USERID, DATECREATED DATECREATED, " + 
				"CASE WHEN NOTE_CLOB IS NULL OR LENGTH(NOTE_CLOB) = 0 THEN TO_CLOB(NOTE) ELSE NOTE_CLOB END \"ATTRIBUTE\" " + 
	...
	public static String newExpenses(Long clientID, String fromDate, String toDate, Vector<Object> param) {
		String sql = 
				"SELECT e.ENTRYID EXPENSEID, d.EXPENSEID SUB_EXPENSEID, e.EMPLOYEEID, e.BILLINGID BILLING_RECID, " + 
	...
	public static String newUpdatedApprovedTimesheetsAndExpenses(Long clientID, String fromDate, String toDate, Vector<Object> param, String[] restriction) {
		String sql = 
				"WITH EXPENSE_CTE AS ( " + 
				"    SELECT e.ENTRYID EXPENSEID, d.EXPENSEID SUB_EXPENSEID, e.EMPLOYEEID, e.BILLINGID BILLING_RECID, b.division, " + 
				"		(SELECT s.RECID FROM TEMPLOYEE_SALARYRECORD s " + 
				"    	WHERE b.EMPLOYEEID = e.EMPLOYEEID AND b.RECRUITER_TEAMID = e.RECRUITER_TEAMID AND b.RECID = e.BILLINGID " + 
				"    	AND b.RECRUITER_TEAMID=s.RECRUITER_TEAMID AND b.EMPLOYEEID=s.EMPLOYEEID AND b.INTERVIEWID=s.INTERVIEWID " + 
				"    	AND e.TODATE BETWEEN s.EFFECTIVE_DATE AND NVL(s.END_DATE,'31-DEC-2999') AND s.APPROVED = 1 AND NVL(s.CLOSED,0)=0) SALARY_RECID, " + 
				"	e.WEEKENDING, d.ITEM, d.EXPENSE, d.QUANTITY, e.TIMECARDID, " + 
				...
				"   AND ROWNUM = 1),'V','V - 3rd Checking','W','W - 2nd Checking','X','X - Savings','Y','Y - Checking','Z','Z - 2nd Savings', " + 
				"    'C','C - Cash Pay Card','') DEDUCTION_CODE, e.RECRUITER_TEAMID TEAMID " + 
				"	FROM TEXPENSE_ENTRY e JOIN TEXPENSE_DETAIL d ON e.ENTRYID = d.ENTRYID AND e.RECRUITER_TEAMID = d.RECRUITER_TEAMID " + 
				"	JOIN TEMPLOYEE_BILLINGRECORD b ON b.EMPLOYEEID = e.EMPLOYEEID AND b.RECRUITER_TEAMID = e.RECRUITER_TEAMID AND b.RECID = e.BILLINGID " + 
				"	WHERE e.RECRUITER_TEAMID = ? AND " + 
				"	e.DATECREATED BETWEEN TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') AND TO_DATE(?,'mm/dd/yyyy hh24:mi:ss') " +
				(restriction!=null && restriction.length > 0 && restriction[0]!=null ? " and b.division=? ":"") +
				"), " + 
				"TIMESHEET_CTE AS ( " + 
				"    SELECT w.timecardid timesheetid, w.employeeid, w.billing_recid, w.salary_recid, w.weekendingdate, w.datecreated, " + 
				"    w.approved_on dateapproved, w.approved_by userapproved, NVL(b.BILLRATE_NAME,'Regular Hours') HOURS_DESCRIPTION, " + 
				"    w.hoursworked, w.reg_hours, w.ot_hours, w.dt_hours, w.comments, s.COMPCODE, " + 
				"    s.SALARY REG_PAY, s.OVERTIME_RATE1 OT_PAY, s.OVERTIME_RATE2 DT_PAY, s.SALARY_PER REG_PAY_RATE_UNIT, " + 
				"    s.OVERTIME_RATE1_PER OT_PAY_RATE_UNIT, s.OVERTIME_RATE2_PER DT_PAY_RATE_UNIT, b.BILL_RATE REG_BILL, " + 
				"    b.OVERTIME_RATE1 OT_BILL, b.OVERTIME_RATE2 DT_BILL, b.BILL_RATE_PER REG_BILL_RATE_UNIT, " + 
				"    b.OVERTIME_RATE1_PER OT_BILL_RATE_UNIT, b.OVERTIME_RATE1_PER DT_BILL_RATE_UNIT, " + 
				...
				"    AND ROWNUM = 1),'V','V - 3rd Checking','W','W - 2nd Checking','X','X - Savings','Y','Y - Checking','Z','Z - 2nd Savings', " + 
				"    'C','C - Cash Pay Card','') DEDUCTION_CODE, w.RECRUITER_TEAMID TEAMID " + 
				...
				"        t.USERAPPROVED, t.HOURS_DESCRIPTION, t.HOURSWORKED, t.REG_HOURS, t.OT_HOURS, t.DT_HOURS, t.COMMENTS, " + 
				"        t.COMPCODE, t.REG_PAY, t.OT_PAY, t.DT_PAY, t.REG_PAY_RATE_UNIT, t.OT_PAY_RATE_UNIT, " + 
				"        t.DT_PAY_RATE_UNIT, t.REG_BILL, t.OT_BILL, t.DT_BILL, t.REG_BILL_RATE_UNIT, t.OT_BILL_RATE_UNIT, " + 
				"        t.DT_BILL_RATE_UNIT, NULL EXPENSEID, NULL ITEM, NULL EXPENSE, NULL QUANTITY " + 
				"        FROM TIMESHEET_CTE t " + 
				"    ) UNION ALL ( " + 
				"        SELECT e.EMPLOYEEID, e.BILLING_RECID, e.SALARY_RECID, e.WEEKENDING, e.TIMECARDID, e.DEDUCTION_CODE, NULL DATECREATED, " + 
				...
				"        NULL DT_PAY_RATE_UNIT, NULL REG_BILL, NULL OT_BILL, NULL DT_BILL, NULL REG_BILL_RATE_UNIT, NULL OT_BILL_RATE_UNIT, " + 
				"        NULL DT_BILL_RATE_UNIT, e.EXPENSEID, e.ITEM, e.EXPENSE, e.QUANTITY " + 
				"        FROM EXPENSE_CTE e " + 
				"    ) " + 
				") " + 
				"ORDER BY EMPLOYEEID, TIMESHEETID, EXPENSEID NULLS FIRST";
		param.add(clientID);
		param.add(fromDate);
		param.add(toDate);
		if(restriction!=null && restriction.length > 0 && restriction[0]!=null) param.add(new Long(restriction[0]));
	...
	public static String candidateOnboardingDocumentList(Long clientID, String[] params, Vector<Object> param) {
		String sql =
				"SELECT CANDIDATEID, ID, FILENAME, " + 
				"(CASE WHEN ONBOARDINGSTATUS>0 THEN (CASE WHEN ONBOARDINGSTATUS=1 THEN " + 
				"    'Verified' ELSE 'Rejected' END) ELSE (CASE WHEN NUM >0 THEN (CASE WHEN DTYPE=1 THEN 'Completed Outside' ELSE " + 
				"    'Pending' END) ELSE (CASE WHEN REQUIRE_RETURN=1 THEN (CASE WHEN ASON IS NOT NULL THEN 'Missing' ELSE 'Saved' END) " + 
				"    ELSE 'Returned Not Required' END) END) END) STATUS, ASSIGNEDON, UPLOADEDON " + 
				"FROM " + 
				"(select " + 
				"(SELECT p.assignedon FROM tpreonboardings p WHERE a.TEAMID = p.TEAMID AND a.CANDIDATEID = p.CANDIDATEID AND a.INTERVIEWID = p.ID) ason, " + 
				"a.onboardingstatus, nvl(b.require_return,0) as require_return, " + 
				"(select count(*) from tcandidate_onboarding_docs where onboardingid=a.id and teamid=a.teamid and nvl(a.DELETED,0)=0) as num, " + 
				"0 as fromtest, b.doctype dtype, a.CANDIDATEID, d.ID, d.filename, a.datecreated assignedon, d.uploadedon " + 
				"    from tcandidate_onboarding a JOIN tonboardings b ON a.TEAMID = b.TEAMID AND a.DOCID = b.ID " + 
				"    LEFT JOIN tcandidate_onboarding_docs d ON a.TEAMID = d.TEAMID AND a.ID = d.ONBOARDINGID " + 
				"    WHERE a.TEAMID = ? AND a.CANDIDATEID = ? " + 
				"    AND d.deleted=0)";
		param.add(clientID);
		param.add(new Long(params[0]));
		return sql;
	...
	public static String userfieldsList(Long clientID, Vector<Object> param) {
		String sql = "SELECT id, fieldname, " + 
					 "(CASE WHEN fieldtypeid = 1 THEN 'String' " + 
					 "		WHEN fieldtypeid = 2 THEN 'Number' " + 
					 "		WHEN fieldtypeid = 3 THEN 'Date' " + 
					 "		WHEN fieldtypeid = 4 THEN 'Date/Time' " + 
					 "		WHEN fieldtypeid = 5 THEN 'Drop Down (Single Selection)' " + 
					 "		WHEN fieldtypeid = 6 THEN 'List (Multiple Selection)' " + 
					 "		WHEN fieldtypeid = 7 THEN 'URL' " + 
					 "		WHEN fieldtypeid = 8 THEN 'Currency' " + 
					 "		WHEN fieldtypeid = 9 THEN 'Percentage' " + 
					 "		WHEN fieldtypeid = 10 THEN 'Text Area' " + 
					 "		WHEN fieldtypeid = 11 THEN 'Sequence' ELSE 'Unknown' END) fieldtype, " +
					 "fieldsize, " + 
					 "(CASE WHEN BITAND(tuf.fieldfor, 1) = 1 THEN 'Contacts' " +
					 "		WHEN BITAND(tuf.fieldfor, 2) = 2 THEN 'Candidates' " +
					 "		WHEN BITAND(tuf.fieldfor, 4) = 4 THEN 'Jobs' " +
					 "		WHEN BITAND(tuf.fieldfor, 8) = 8 THEN 'Companies' " +
					 "		WHEN BITAND(tuf.fieldfor, 112) > 0 THEN 'Activities' " +
					 "		WHEN BITAND(tuf.fieldfor, 128) = 128 THEN 'HR' " +
					 "		WHEN BITAND(tuf.fieldfor, 256) = 256 THEN 'Opportunity' ELSE 'Unknown' END) fieldfor, " +
					 "(CASE WHEN mandatory = 1 THEN 'Yes' " +
					 "		ELSE 'No' END) mandatory, " +
					 "ttabs.tabname " +
					 "FROM tuserfields tuf LEFT JOIN tuserfield_tabs ttabs " + 
					 "ON tuf.tabid = ttabs.tabid AND tuf.teamid = ttabs.teamid " +
					 "WHERE tuf.teamid = ? ";
		param.add(clientID);
		return sql;
	}

...
public class JobUserDao extends AbstractJobDivaDao {
	...
	public Boolean assignUserToJob(JobDivaSession jobDivaSession, Long rfqid, Long recruiterid, List<Long> roleIds, Integer jobstatus) throws Exception {
		...
		if (customerID > 0) {
			int RoleID = 950;
			if (roleIds.contains(996l))
				RoleID = 996; // lead sales
			else if (roleIds.contains(999l))
				RoleID = 999; // sales
			else if (roleIds.contains(998l))
				RoleID = 998; // lead recruiter
			else if (roleIds.contains(997l))
				RoleID = 997; // recruiter
		...
		// send Email
		SendEmailJobAssignment(jdbcTemplate, rfqid, jobDivaSession.getTeamId(), recruiterid, assignedTitle, jobDivaSession.getEnvironment().toString(), recruiterEmail, recruiterName, EMAIL_OPTION_ASSIGN_USER, receive_email == 1,
	...
	private void SendEmailJobAssignment(JdbcTemplate jdbcTemplate, Long rfqid, Long teamId, Long recruiterid, String assignedTitle, String env, String recruiterEmail, String recruiterName, int email_option, boolean sendEmail, long recruiterId2)
		...
		String sqlStr = "select rfq.rfqno_team,rfq.rfqrefno,rfq.rfqtitle,rfq.department,rfq.city,decode(countries.defaultdisplay,1,states.state_name, states.state_abbr) as jobstate, countries.country as countryname,rfq.jobdescription,"
				...
				+ "decode(rfq.rateper,'h','hour','d','day','y','year',(select x.name from trateunits x where rfq.rateper=x.unitid and x.teamid=rfq.teamid and x.ratetype=0 and x.deleted=0 )) as payUnit, " + // "ASCII(rfq.rateper)
																																																				// AS
																																																				// pay_UnitCode,
																																																				// "
																																																				// +
				"currency.id as pay_currencyid, currency.name as pay_currencyname, currency.symbol as pay_currencysign,rfq.startdate,rfq.enddate," + "rfq.positions, rfq.contract as contractCode,"
				+ "(select x.name from trfq_position_types x where rfq.contract=x.id and rfq.teamid=x.teamid) as Contract,  " + // rfq.contract
				"(select x.region_code from tteam x where x.id=rfq.teamid) as region_code," + "rfq.instruction from trfq rfq,tcustomer cus, tstates states, tcountries countries, tcurrency currency "
				...
				if (hasJobInfo && sendEmail) {
					try {
						...
						if (email_option == EMAIL_OPTION_ASSIGN_USER) {
							...
							emailBody = "This is to inform you that you have just been assigned " + (assignedTitle.length() > 0 ? "as " + assignedTitle : "") + " to Job Reference # <a href='" + emailLocation + "/employers/open_rfq.jsp?rfqid=" + rfqid
									+ "'>" + rfqno + "</a> (" + rfqtitle + ")" + //
							"<br><br>" + strJobInfo; //
	...
	public Boolean unassignUserFromJob(JobDivaSession jobDivaSession, Long jobId, Long recruiterid) throws Exception {
		...
		//
		// Send Email
		SendEmailJobAssignment(jdbcTemplate, jobId, jobDivaSession.getTeamId(), recruiterid, "", jobDivaSession.getEnvironment().toString(), recruiterEmail, recruiterName, EMAIL_OPTION_UNASSIGN_USER, receive_email == 1,
				jobDivaSession.getRecruiterId());
		return true;
	}
	...
	public Boolean updateUserRoleForJob(JobDivaSession jobDivaSession, Long jobId, Long recruiterid, List<Long> roleIds) throws Exception {
		...
		//
		String sqlUpdate = "Update tRecruiterRFQ Set recruiter=?,lead_recruiter=?,sales=?,lead_sales=? " + "Where rfqid=? and recruiterid=? and teamid=? ";

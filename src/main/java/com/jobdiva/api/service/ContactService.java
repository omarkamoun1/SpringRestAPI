		...
		try {
			...
			return inTransaction(new TransactionCallback<Long>() {
				...
				public Long doInTransaction(TransactionStatus status) {
					try {
						Long contactId = contactDao.createContact(jobDivaSession, company, firstname, lastname, title, department, phones, addresses, email, alternateemail, types, owners, reportsto, primary, assistantname, assistantemail,
								assistantphone, assistantphoneextension, subguidelines, maxsubmittals, references, drugtest, backgroundcheck, securityclearance);
						//
	...
	public Boolean updateContact(JobDivaSession jobDivaSession, Long contactid, String firstname, String lastname, String title, Long companyid, String department, ContactAddress[] addresses, PhoneType[] phones, Long reportsto, Boolean active,
		...
		try {
			...
			return inTransaction(new TransactionCallback<Boolean>() {
				...
				public Boolean doInTransaction(TransactionStatus status) {
					try {
						Boolean success = contactDao.updateContact(jobDivaSession, contactid, firstname, lastname, title, companyid, department, addresses, phones, reportsto, active, primary, email, alternateemail, assistantname, assistantemail,
								assistantphone, assistantphoneextension, subguidelines, maxsubmittals, references, drugtest, backgroundcheck, securityclearance, userfields, owners, types);
						//
